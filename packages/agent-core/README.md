# @archymedes/nova-core

The provider-neutral coding-agent core behind [`archymedes-nova`](https://www.npmjs.com/package/archymedes-nova): a bounded tool-use runtime, local and sandboxed workspace backends, model adapters for Anthropic/OpenAI-compatible providers, and multi-currency cost accounting. Use this package directly if you're embedding an agent loop in your own tool rather than running Nova's CLI.

```bash
npm install @archymedes/nova-core
```
...
```ts
import { resolveProvider } from "@archymedes/nova-core";
