...
import type { AgentModelRequest, AgentModelTurn, AgentTurnProvider } from "@archymedes/nova-core/agent-runtime";
import type { NovaEvent } from "@archymedes/nova-core/nova-cli/agent";
import { enqueueJob, getJob, newJobId, readJobLog, resolveJobApproval } from "@archymedes/nova-core";
import { saveSession, newSessionId } from "@archymedes/nova-core/nova-cli/session";
import {
...
describe("the detached approval prompt directly", () => {
  it("gives up after its timeout without ever being answered", async () => {
    const job = await enqueueJob(root, { id: newJobId(), objective: "x", logPath: "l" });
    await import("@archymedes/nova-core").then((core) => core.claimJob(root, "worker-1", 60_000));
