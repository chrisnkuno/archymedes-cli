...
describe("self-update version invariants", () => {
  ...
  it("builds argv arrays rather than shell commands and pins the checked version", () => {
    expect(updateCommand("npm", "1.2.3")).toEqual({
      command: "npm",
      args: ["install", "--global", "archymedes-nova@1.2.3"],
    });
    expect(updateCommand("pnpm", "1.2.3").args).toEqual(["add", "--global", "archymedes-nova@1.2.3"]);
    expect(updateCommand("yarn", "1.2.3").args).toEqual(["global", "add", "archymedes-nova@1.2.3"]);
    expect(updateCommand("bun", "1.2.3").args).toEqual(["add", "--global", "archymedes-nova@1.2.3"]);
...
describe("package-manager detection", () => {
  ...
  it("reads package-manager launch metadata and installed paths", () => {
    ...
    expect(detectPackageManager({ environment: {}, modulePath: "file:///home/me/.bun/install/global/node_modules/archymedes-nova/dist/nova.js" })).toBe("bun");
    expect(detectPackageManager({ environment: {}, modulePath: "file:///opt/pnpm/global/5/node_modules/archymedes-nova/dist/nova.js" })).toBe("pnpm");
...
describe("registry checks", () => {
  it("reads the exact package's latest valid version", async () => {
    ...
    expect(String(fetchImpl.mock.calls[0][0])).toContain("archymedes-nova/latest");
  });
...
describe("self-update workflow", () => {
  ...
  it("installs the exact checked version outside the caller's workspace without a shell", async () => {
    ...
    expect(args).toEqual(["add", "--global", "archymedes-nova@0.2.0"]);
  ...
  it("propagates installer failure and provides a safe manual retry", async () => {
    ...
    expect(test.stderr.join("")).toContain("npm install --global archymedes-nova@0.2.0");
  });
});
