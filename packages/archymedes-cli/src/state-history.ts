import {
  tryConnectArchymedesState,
  type ArchymedesStateClient,
  type StateIndexReport,
  type StateSearchHit,
  type StateSessionSummary,
} from "@archymedes/core/cli/state-client";

type StateReader = Pick<ArchymedesStateClient, "rebuild" | "search" | "sessions" | "close">;
...
/**
 * The CLI-facing lifecycle for archymedes-state.
 *
...
export class CliStateHistory {
  ...
  constructor(
    ...
    private readonly connectState: ConnectState = async (environment) => tryConnectArchymedesState({ environment: environment as NodeJS.ProcessEnv }),
  ) {}
