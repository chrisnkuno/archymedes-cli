...
describe("Defender feed CLI policy", () => {
  ...
  it("accepts only bounded key-id/string mappings", () => {
    expect(defenderFeedKeys({ ARCHYMEDES_DEFENDER_BRAIN_PUBLIC_KEYS: JSON.stringify({ "key-1": "public", "bad id": "ignored", huge: "x".repeat(5_000) }) })).toEqual({ "key-1": "public" });
    expect(defenderFeedKeys({ ARCHYMEDES_DEFENDER_BRAIN_PUBLIC_KEYS: "not json" })).toEqual({});
  });
});
