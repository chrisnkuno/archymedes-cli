...
import { SECRET_PATTERNS } from "@archymedes/core/cli/secret-scan";
...
export function sessionExportPayload(record: SessionRecord, format: ExportFormat): Record<string, unknown> {
  ...
  const base = {
    schema: `archymedes-session-export/v1`,
    sessionId: record.id,
...
/** Writes a private-by-default, redacted artifact under the project's .archymedes directory. */
export async function exportSession(record: SessionRecord, format: ExportFormat): Promise<string> {
  const directory = path.join(record.root, ".archymedes", "exports");
