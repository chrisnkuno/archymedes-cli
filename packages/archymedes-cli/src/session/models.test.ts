...
describe("the model catalog", () => {
  ...
  it("never lists a model whose provider has no key, except Ollama which needs none", () => {
    ...
    expect(unconfigured.map((entry) => entry.provider).sort()).toEqual(["archymedes-cloud", "deepseek", "free", "google", "groq", "mistral", "openai", "openai-compatible", "xai"]);
  ...
  it("still offers Ollama's local model when nothing else is configured", () => {
    ...
    expect(catalog.unconfigured).toHaveLength(10);
  });
