 ...
 *
 * `archymedes update` has always existed, which means updating was something you had to remember to do —
...
export function readAutoUpdateMode(environment: Record<string, string | undefined>): AutoUpdateMode {
  const raw = environment.ARCHYMEDES_AUTO_UPDATE?.trim().toLowerCase();
 ...
 * while developing Archymedes itself.
 */
...
function stateFile(environment: Record<string, string | undefined>): string {
  return path.join(archymedesConfigDirectory(environment), "update-check.json");
}
...
export function updateNotice(version: string, currentVersion: string): string[] {
  return [
    `Archymedes ${version} is available — you are on ${currentVersion}.`,
...
export async function runAutoUpdate(options: {
  ...
  if (decision.action === "install" && options.install) {
    ...
    return { notice: [`Could not install Archymedes ${decision.version} automatically. Run /update to see why.`], decision };
  }
