...
export const GUIDE_TOPICS: GuideTopic[] = [
  ...
  },
  {
    ...
    summary: "Topping up your Archymedes credit without leaving the prompt.",
    body: [
      "/pay creates a payment and gives you a link and a short code. You pay on Circuit Pay's own page — Archymedes never sees your card, your PIN or your mobile-money confirmation, and never asks for them.",
      "Nothing is charged until you answer the confirmation, and the amount you approve is the amount that appears on the checkout. Amounts are whole RWF: Archymedes refuses a decimal rather than rounding money you typed.",
      "While it waits, Ctrl+C returns you to the prompt without cancelling the payment. If it is still unconfirmed when you stop waiting, Archymedes says so and keeps the reference rather than calling it failed — a payment that has not been confirmed is not a payment that has failed, and paying twice is the mistake worth designing against. /pay status <reference> picks it up later.",
      "/pay on its own shows the balance. When CircuitNotion billing is configured, Archymedes also checks after a turn and calmly warns if the confirmed balance is low, below the critical 500 RWF level, or has fallen unusually fast. If that endpoint is unavailable, /balance <amount> stores the balance you see in your account and tracks a clearly labelled local estimate by subtracting measured token costs after completed turns. /balance clear returns to the endpoint. Before a demanding task, Archymedes compares its token-based estimate with whichever balance source you chose and stops before contacting the model when even the conservative estimate cannot fit.",
      "Paying needs ARCHYMEDES_BILLING_URL and ARCHYMEDES_BILLING_KEY set in /settings; without both, /pay says so instead of half-working.",
    ],
    examples: [
