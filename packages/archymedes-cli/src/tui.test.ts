...
describe("Archymedes activity", () => {
  ...
  it("falls back safely for invalid frame indexes", () => {
    expect(archymedesSpinnerFrame(Number.NaN)).toBe(archymedesSpinnerFrame(0));
    expect(archymedesSpinnerFrame(-4)).toBe(archymedesSpinnerFrame(0));
  });
  ...
  it("shows the operation phase in the status line", () => {
    const line = formatStatusLine({ mode: "build", spinnerGlyph: archymedesSpinnerFrame(3), elapsedMs: 4_200, toolCalls: 1, tokens: 1_200, cost: "$0.0120", phase: "operation", operation: "edit_file" }, 80, "none");
