...
import type { AgentModelRequest, AgentModelTurn, AgentTurnProvider } from "@archymedes/core/agent-runtime";
...
function scriptedModel(turns: Array<Partial<AgentModelTurn>>): AgentTurnProvider & { requests: AgentModelRequest[] } {
  ...
  return {
    ...
    async complete(request) {
      ...
      index += 1;
      return { responseId: `resp_${index}`, model: "archymedes-test", finishReason: "stop", content: "Done.", toolCalls: [], usage, ...turn } as AgentModelTurn;
    },
  };
}
...
beforeEach(async () => {
  root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-worker-"));
