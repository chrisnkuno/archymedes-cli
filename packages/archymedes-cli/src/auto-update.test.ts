...
beforeEach(async () => {
  configDir = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-autoupdate-"));
});
...
describe("when Archymedes looks for a new version", () => {
...
describe("what it does with a version it already knows about", () => {
  ...
  it("never offers a downgrade, which is the normal case while developing Archymedes itself", () => {
...
describe("the setting", () => {
  it("defaults interactive sessions to installing without another acceptance prompt", () => {
    ...
    expect(readAutoUpdateMode({ ARCHYMEDES_AUTO_UPDATE: "" })).toBe("install");
  });

  it("keeps malformed explicit settings notification-only", () => {
    expect(readAutoUpdateMode({ ARCHYMEDES_AUTO_UPDATE: "nonsense" })).toBe("check");
  });

  it("reads the spellings people actually type", () => {
    for (const value of ["off", "false", "0", "no", "OFF"]) expect(readAutoUpdateMode({ ARCHYMEDES_AUTO_UPDATE: value }), value).toBe("off");
    for (const value of ["check", "notify", "CHECK"]) expect(readAutoUpdateMode({ ARCHYMEDES_AUTO_UPDATE: value }), value).toBe("check");
    for (const value of ["install", "auto", "on", "true", "1", "yes", "Install"]) expect(readAutoUpdateMode({ ARCHYMEDES_AUTO_UPDATE: value }), value).toBe("install");
  });
});
...
describe("the stamp on disk", () => {
  ...
  it("survives a config directory it cannot write to", async () => {
    ...
    await expect(writeAutoUpdateState({ ARCHYMEDES_CONFIG_DIR: path.join(blocker, "archymedes") }, { lastCheckedAt: 1 })).resolves.toBeUndefined();
    expect(await readAutoUpdateState({ ARCHYMEDES_CONFIG_DIR: path.join(blocker, "archymedes") })).toBeNull();
  });
});
