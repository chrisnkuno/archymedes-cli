 ...
 * validates a record, `ArchymedesAgent.resume` restores a transcript — and every one of them can pass
 ...
 *
 * One-shot mode (`archymedes --resume "…"`) rather than the REPL, deliberately: it exercises the same
...
describe("resuming a past session", () => {
  ...
  beforeEach(async () => {
    ...
    cwd = await mkdtemp(path.join(os.tmpdir(), "archymedes-resume-"));
    // Isolated from the developer's own settings, which could otherwise supply a real API key.
    configDir = await mkdtemp(path.join(os.tmpdir(), "archymedes-resume-config-"));
  });
  ...
  async function sessionIds(): Promise<string[]> {
    const files = await readdir(path.join(cwd, ".archymedes", "sessions")).catch(() => [] as string[]);
  ...
  it("refuses a session recorded against a different project root", async () => {
    ...
    const file = path.join(cwd, ".archymedes", "sessions", `${id}.json`);
