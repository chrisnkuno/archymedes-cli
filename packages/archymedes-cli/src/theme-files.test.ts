...
beforeEach(async () => {
  root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-theme-project-"));
  home = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-theme-home-"));
...
describe("finding themes on disk", () => {
  ...
  it("keeps the two scopes in separate directories", () => {
    expect(themeDirectory("project", root, environment)).toBe(path.join(root, ".archymedes", "themes"));
