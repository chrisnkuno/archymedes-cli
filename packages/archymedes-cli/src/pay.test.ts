...
describe("what a money screen says", () => {
  ...
  it("says Archymedes never sees the payment instrument, before anything is created", () => {
  ...
  it("names both settings when there is no gateway to call", () => {
    expect(BILLING_NOT_CONFIGURED.join(" ")).toContain("ARCHYMEDES_BILLING_URL");
    expect(BILLING_NOT_CONFIGURED.join(" ")).toContain("ARCHYMEDES_BILLING_KEY");
  });
});
