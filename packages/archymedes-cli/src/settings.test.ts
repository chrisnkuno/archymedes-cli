...
describe("Archymedes settings", () => {
  it("uses native config locations on each desktop platform", () => {
    expect(settingsDirectory({ APPDATA: "C:\\Users\\Archymedes\\AppData\\Roaming" }, "win32")).toContain(path.join("Archymedes"));
    expect(settingsDirectory({}, "darwin")).toContain(path.join("Library", "Application Support", "Archymedes"));
    expect(settingsDirectory({ XDG_CONFIG_HOME: "/tmp/config" }, "linux")).toBe(path.join("/tmp/config", "archymedes"));
  });

  it("persists only allowed settings and lets process environment override them", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-settings-")); roots.push(root);
    const environment = { ARCHYMEDES_CONFIG_DIR: root };
  ...
  it("takes a location as a country code and normalises it", () => {
    expect(validateSetting("ARCHYMEDES_COUNTRY", "rw")).toBe("RW");
    expect(validateSetting("ARCHYMEDES_COUNTRY", " eg ")).toBe("EG");
  });
  ...
  it("refuses a country it cannot price in, rather than saving a setting that does nothing", () => {
    ...
    expect(() => validateSetting("ARCHYMEDES_COUNTRY", "ZZ")).toThrow("No local currency is known");
    expect(() => validateSetting("ARCHYMEDES_COUNTRY", "Rwanda")).toThrow("two-letter");
  });

  it("takes an explicit display currency, and refuses one it cannot convert to", () => {
    expect(validateSetting("ARCHYMEDES_CURRENCY", "rwf")).toBe("RWF");
    expect(() => validateSetting("ARCHYMEDES_CURRENCY", "XYZ")).toThrow("not a currency");
  });

  it("accepts only real providers as the remembered default, normalised to lower case", () => {
    // resolveProvider ignores an unrecognised ARCHYMEDES_PROVIDER rather than failing to start, so a typo
    // entered here would otherwise be accepted and then silently do nothing.
    expect(validateSetting("ARCHYMEDES_PROVIDER", "OpenAI")).toBe("openai");
    expect(validateSetting("ARCHYMEDES_PROVIDER", "anthropic")).toBe("anthropic");
...
describe("the settings menu with a chooser", () => {
  ...
  it("offers every field as a row, each showing what it is currently set to", async () => {
    ...
    await runSettingsMenu({ ARCHYMEDES_COUNTRY: "RW" }, { ...silent, choose });
  ...
  it("picks an enumerated value from a list instead of asking it to be typed", async () => {
    ...
    expect(result.ARCHYMEDES_COUNTRY).toBe("RW");
  ...
  it("offers a way to clear an enumerated setting, and clearing it works", async () => {
    ...
    const result = await runSettingsMenu({ ARCHYMEDES_COUNTRY: "RW" }, { ...silent, choose });
    expect(result.ARCHYMEDES_COUNTRY).toBeUndefined();
  });
  ...
  it("treats dismissing the top menu as leaving it, saving what was already changed", async () => {
    ...
    const result = await runSettingsMenu({}, { ...silent, choose });
    expect(result.ARCHYMEDES_PROVIDER).toBe("openai");
  });
  ...
  it("reopens the field list where the user was, not back at the top", async () => {
    ...
    await runSettingsMenu({}, { ...silent, choose });
    const providerRow = SETTING_FIELDS.findIndex((field) => field.key === "ARCHYMEDES_PROVIDER");
...
describe("the first run someone actually sees", () => {
  ...
  it("asks only which provider key you have, not all twenty-four settings", async () => {
    // The obstacle this removes: someone installing Archymedes to use Claude was shown a 24-item list
...
/**
 * The one field whose valid answers Archymedes cannot know.
 *
 * A country code is a fact this build holds; a model id is not. It belongs to the provider, it
 * changes with no release of Archymedes, and the key needed to ask for it has — by the time this field
...
describe("picking a model in the settings menu", () => {
  ...
  it("covers every provider Archymedes can be configured to use", () => {
