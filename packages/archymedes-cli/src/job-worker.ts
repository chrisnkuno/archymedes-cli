...
import { isWanderObjective, WANDER_LAB_FILES } from "@archymedes/core/wander";
import {
  appendJobLog,
  claimJob,
  consumeJobApproval,
 ...
 * worker dying. This file is the worker: it claims one job, runs the same `ArchymedesAgent` the
 * interactive CLI runs, and reports back through the same store. Nothing about the agent loop is
 * reimplemented; a fix or a regression in how Archymedes executes a turn applies identically whether the
...
/**
 * One `ArchymedesEvent` in, zero or more finished log lines out.
 *
 ...
 */
export function stepJobLog(state: JobLogState, event: ArchymedesEvent): { state: JobLogState; lines: string[] } {
...
async function workspaceDispose(workspace: ArchymedesWorkspace | undefined): Promise<void> {
