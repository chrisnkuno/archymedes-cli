...
describe("exchange rates", () => {
  ...
  it("supports an auditable manual rate for any currency pair", () => {
    expect(readFxRates({ ARCHYMEDES_FX_FROM: "USD", ARCHYMEDES_FX_TO: "EGP", ARCHYMEDES_FX_RATE: "48.5", ARCHYMEDES_FX_ASOF: "2026-08-08" })[0]).toEqual({
      from: "USD", to: "EGP", rate: 48.5, asOf: "2026-08-08", source: "ARCHYMEDES_FX_RATE",
    });
  });
});
...
describe("main() — branches that resolve before any interactive input is needed", () => {
  ...
  beforeEach(async () => {
    ...
    // `ARCHYMEDES_CONFIG_DIR`, which sends `loadSettings` to the real per-user config file, so
    // "unconfigured" meant "unconfigured unless whoever runs this suite actually uses Archymedes". It
    // passed on CI and on a fresh checkout and failed for anyone who had run `archymedes settings` —
    // the one population most likely to be running the tests.
    process.env.ARCHYMEDES_CONFIG_DIR = await mkdtemp(path.join(os.tmpdir(), "archymedes-main-config-"));
  });
  ...
  async function run(args: string[]): Promise<{ code: number; stdout: string; stderr: string }> {
    ...
    process.argv = ["node", "archymedes", ...args];
    const { main } = await import("./archymedes");
  ...
  it("--version prints the bundled package version without provider setup", async () => {
    ...
    expect(code).toBe(0);
    expect(stdout).toMatch(/^archymedes \d+\.\d+\.\d+\n$/);
  });
  ...
  it("refuses to run with no model provider configured, and says so on stderr", async () => {
    ...
    expect(code).toBe(1);
    expect(stderr).toContain("Archymedes is not configured.");
  });
  ...
  it("never sends a one-shot slash command to the model", async () => {
    ...
    expect(code).toBe(2);
    expect(stderr).toContain("Slash commands run inside an interactive Archymedes session");
