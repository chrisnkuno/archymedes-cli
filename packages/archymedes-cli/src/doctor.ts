...
/**
 * `archymedes --doctor`: probes every network endpoint the CLI depends on and reports exactly which
 * one is failing, in seconds.
 *
 * The most common support report is "Archymedes says there is no internet, but the internet works."
...
async function probe(endpoint: DoctorEndpoint, options: DoctorOptions): Promise<DoctorProbe> {
  ...
  // is set, and probing anyway would report a "failure" that has no bearing on using Archymedes.
...
export function doctorReport(probes: readonly DoctorProbe[], context: DoctorReportContext): Record<string, unknown> {
  return {
    schema: "archymedes-doctor-report/v1",
