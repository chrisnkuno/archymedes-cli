/**
 * Every non-ASCII character Archymedes draws, in one table, with an ASCII twin for terminals that cannot
 * render it.
 *
 ...
 * seen the wrong answer needs one switch that ends the argument — `ARCHYMEDES_GLYPHS=ascii` (or `unicode`)
 * is that switch.
 *
...
export function detectGlyphMode(
  ...
  const forced = environment.ARCHYMEDES_GLYPHS?.trim().toLowerCase();
  ...
  if (environment.ARCHYMEDES_ASCII?.trim() && environment.ARCHYMEDES_ASCII !== "0") return "ascii";
