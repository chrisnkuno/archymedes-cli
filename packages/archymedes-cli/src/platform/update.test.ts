...
import {
  ...
  fetchLatestVersion,
  ARCHYMEDES_CLI_PACKAGE,
  parseSemver,
  runSelfUpdate,
  updateCommand,
} from "./update";

function registryResponse(version: string, name = ARCHYMEDES_CLI_PACKAGE): Response {
...
describe("self-update version invariants", () => {
  ...
  it("builds argv arrays rather than shell commands and pins the checked version", () => {
    expect(updateCommand("npm", "1.2.3")).toEqual({
      command: "npm",
      args: ["install", "--global", "archymedes-cli@1.2.3"],
    });
    expect(updateCommand("pnpm", "1.2.3").args).toEqual(["add", "--global", "archymedes-cli@1.2.3"]);
    expect(updateCommand("yarn", "1.2.3").args).toEqual(["global", "add", "archymedes-cli@1.2.3"]);
    expect(updateCommand("bun", "1.2.3").args).toEqual(["add", "--global", "archymedes-cli@1.2.3"]);
...
describe("package-manager detection", () => {
  ...
  it("reads package-manager launch metadata and installed paths", () => {
    ...
    expect(detectPackageManager({ environment: {}, modulePath: "file:///home/me/.bun/install/global/node_modules/archymedes-cli/dist/archymedes.js" })).toBe("bun");
    expect(detectPackageManager({ environment: {}, modulePath: "file:///opt/pnpm/global/5/node_modules/archymedes-cli/dist/archymedes.js" })).toBe("pnpm");
...
describe("registry checks", () => {
  it("reads the exact package's latest valid version", async () => {
    ...
    expect(String(fetchImpl.mock.calls[0][0])).toContain("archymedes-cli/latest");
  });

  it("rejects package substitution, invalid versions, insecure registries, and HTTP failures", async () => {
    await expect(fetchLatestVersion({ fetchImpl: async () => registryResponse("0.2.0", "other") })).rejects.toThrow("invalid Archymedes package record");
    await expect(fetchLatestVersion({ fetchImpl: async () => registryResponse("latest") })).rejects.toThrow("invalid Archymedes package record");
    await expect(fetchLatestVersion({ environment: { ARCHYMEDES_UPDATE_REGISTRY: "http://registry.example" } })).rejects.toThrow("must use HTTPS");
...
describe("self-update workflow", () => {
  ...
  it("checks without installing and prints the explicit next action", async () => {
    ...
    expect(test.spawnImpl).not.toHaveBeenCalled();
    expect(test.stdout.join("")).toContain("archymedes update");
  });
  ...
  it("installs the exact checked version outside the caller's workspace without a shell", async () => {
    ...
    expect(args).toEqual(["add", "--global", "archymedes-cli@0.2.0"]);
    expect(options).toMatchObject({ shell: false, stdio: "inherit" });
    expect(options.cwd).not.toBe(process.cwd());
    expect(test.stdout.join("")).toContain("Restart Archymedes");
  });
  ...
  it("propagates installer failure and provides a safe manual retry", async () => {
    ...
    expect(test.stderr.join("")).toContain("npm install --global archymedes-cli@0.2.0");
  });
});
