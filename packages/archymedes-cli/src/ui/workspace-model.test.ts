...
describe("building panes from tabs", () => {
  const views: TabView[] = [
    { id: 1, title: "archymedes", status: "idle", unread: 0, active: true, model: "claude-sonnet-5", backend: "local", cost: "$0.02" },
  ...
  it("numbers the bar from one, and marks exactly one cell active", () => {
    ...
    expect(tabs.map((tab) => tab.label)).toEqual(["1 archymedes", "2 sandbox"]);
