import { GUIDE_TOPICS, wrapText, type GuideTopic } from "../render/guide";
import { NO_COLOR_PALETTE, type Palette } from "../theme/theme";
import { visibleWidth } from "../text/text-width";
import { joinHorizontal, padToWidth as pad, scrollIndicator, scrollPercent, sliceToWidth } from "../render/tui";
import { applyViewport, newViewport, visibleLines } from "../terminal/viewport";
...
export function composeGuideFrame(state: GuideBrowserState): GuideRow[] {
  ...
  const rows: GuideRow[] = [];
  rows.push({
    text: pad(compact ? ` archymedes guide · ${topic?.title ?? "no matches"}` : " archymedes guide", columns),
    bold: true,
    color: theme.primary,
  });
