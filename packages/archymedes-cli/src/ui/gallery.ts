import { renderBanner, renderTagline } from "../render/banner";
import { type ColorDepth } from "../text/color-depth";
import { barChart, heatStrip, lineChart, plotSeries, scatterChart, waveLine } from "../render/charts";
...
import { renderCode, renderDiff, diffLines } from "../render/code-view";
import { renderDropup } from "./dropup";
import { renderExpandableList } from "../render/expandable";
 ...
 *
 * So this is a screen, not a test: `archymedes --gallery`, and `archymedes --gallery --ascii --no-color` for the
...
export function renderGallery(options: GalleryOptions): string {
  ...
  parts.push(section("progress and status", style, [
    `spinner   ${Array.from({ length: 6 }, (_, index) => archymedesSpinnerFrame(index, glyphs)).join("  ")}`,
    ...
    formatStatusLine(
      { mode: "build", spinnerGlyph: archymedesSpinnerFrame(2, glyphs), elapsedMs: 12_400, toolCalls: 7, tokens: 24_000, cost: "$0.14", phase: "operation", operation: "run_command", steps: { done: 4, total: 9, label: "plan" } },
      width, depth, glyphs,
    ),
    ...(() => {
