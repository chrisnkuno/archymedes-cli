...
import { buildFileTree, type FileNode } from "@archymedes/core/cli/file-tree";
import { visibleWidth } from "../text/text-width";
import { NO_COLOR_PALETTE, type Palette } from "../theme/theme";
import { joinHorizontal, paginator, padToWidth as pad } from "../render/tui";

/**
 * The project, as something you look around in rather than type paths into.
 *
 * Archymedes already completes `@path` mentions from a flat file list — real, but blind: typing `@ap` and
...
export { directorySummary, type FileNode } from "@archymedes/core/cli/file-tree";
...
export function composeFileFrame(state: FileBrowserState, preview: FilePreview, glyphs: GlyphSet = UNICODE_GLYPHS): FileRowStyle[] {
  ...
  out.push({ text: pad(` archymedes files · ${current ? current.node.path : "no matches"}`, columns), bold: true, color: theme.primary });
