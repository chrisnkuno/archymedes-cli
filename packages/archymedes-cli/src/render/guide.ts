...
import { BOLD, DIM, paint, paintAll } from "../text/ansi";
...
export const GUIDE_TOPICS: GuideTopic[] = [
  {
    ...
    summary: "What Archymedes is, and the first thing to type.",
    body: [
      "Archymedes is a coding agent that works in a scrolling terminal transcript. You type what you want in plain language; it reads files, runs commands and edits code, and shows you what it did as it goes.",
    ...
    examples: [
      { input: "fix the failing test in cart.ts", effect: "an ordinary request; Archymedes reads, edits and runs the tests" },
  ...
  },
  {
    ...
    body: [
      ...
      "Only the tab in front runs. Switching away does not leave a turn working in the background — it pauses that piece of work and hands the terminal to another one. That is a deliberate limit rather than a missing feature: a terminal transcript has one bottom, and two agents printing into it at once would interleave into something neither of them said. (Archymedes's desktop window is different — it has a transcript per tab, so its tabs really do run at the same time.)",
  ...
  },
  {
    ...
    body: [
      "By default Archymedes edits the directory you started it in. Started with --sandbox e2b it works inside a throwaway remote machine instead, and --sandbox docker uses a local container. Files written there never touch your disk unless you ask for them.",
      "Sandboxes are per tab, so 'try this somewhere I don't mind breaking' is one command rather than a second terminal.",
      "One thing worth knowing: background jobs always run on your machine, even when the session that started them is sandboxed. Archymedes says so each time you start one.",
    ],
    examples: [
      { input: "archymedes --sandbox e2b", effect: "the whole session runs in a remote sandbox" },
  ...
  {
    id: "reading",
    title: "Reading what Archymedes did",
    summary: "Code, diffs, test results, and folded detail.",
    body: [
      "When Archymedes writes or edits a file it shows the code, numbered, under the tool line. When it runs tests it re-lays the output into sections with the failures pulled out. When output is long it is folded rather than truncated, and the whole of it is one command away.",
  ...
  },
  {
    ...
    body: [
      "Telling Archymedes the same thing every session costs you tokens and patience. A remembered fact is prepended to the conversation instead.",
  ...
  },
  {
    ...
    examples: [
      ...
      { input: "archymedes --budget 20", effect: "approve and enforce a cap" },
  ...
  {
    id: "balance",
    title: "Tracking a spend balance",
    summary: "A local budget the prompt draws down as you work.",
    body: [
      "Archymedes has no hosted wallet. /balance sets a figure — the amount you have decided this project can spend — and after every completed turn Archymedes subtracts that turn's measured cost from it. It is an estimate you own, not a provider statement.",
      "The balance is kept in whatever currency your session shows costs in. /balance 5000 uses that currency; /balance 50 usd or /balance 5000 inr sets it explicitly. /balance on its own shows what remains and what this session has used so far. /balance clear stops tracking.",
      "Once a balance is being tracked, Archymedes warns calmly when it gets low, drops below the critical threshold, or falls unusually fast between turns. Before a demanding task it compares its token-based estimate with the balance and stops before contacting the model when even the conservative estimate cannot fit.",
      "The thresholds default to small USD amounts converted to your currency; ARCHYMEDES_LOW_BALANCE and ARCHYMEDES_CRITICAL_BALANCE override them in /settings.",
    ],
    examples: [
      { input: "/balance 5000", effect: "track a 5,000 balance in the display currency" },
      { input: "/balance 50 usd", effect: "track a balance in a specific currency" },
      { input: "/balance", effect: "what remains, and what this session has used" },
      { input: "/balance clear", effect: "stop tracking a balance" },
    ],
    covers: ["/balance", "/cost"],
  },
  {
    id: "updating",
    title: "Keeping Archymedes current",
    summary: "Automatic updates by default, with explicit check-only and off controls.",
    body: [
      "Archymedes looks for a new version once a day, at the moment a session starts and never in the middle of one. The check is bounded at three seconds and its failure is silent: an unreachable registry costs one attempt a day, not a slow prompt.",
      "By default Archymedes installs a newer release automatically through whichever package manager installed it; it does not wait for another acceptance prompt. /update check changes that to notification-only behavior, and /update off stops it looking at all.",
  ...
  },
  {
    ...
    examples: [
      ...
      { input: "/find TypeError", effect: "search this tab's history; /find again for the next match, Esc to return" },
      { input: "/pager", effect: "open the whole transcript in less, to search, select or save" },
    ...
    covers: ["/theme", "/layout", "/find", "/pager"],
  },
  {
    id: "extend",
    ...
    body: [
      ...
      "Mentioning an image — `what is wrong in @screenshots/bug.png?` — sends the picture itself with that request (PNG, JPEG, GIF or WebP, up to four, 5 MB each, from a local workspace). The model must accept images; the saved conversation keeps a note of the attachment rather than the image.",
...
export function renderGuideIndex(style: SectionStyle): string {
  ...
  return [
    rule(style, { label: "archymedes guide", tone: "accent" }),
    ...rows,
    "",
