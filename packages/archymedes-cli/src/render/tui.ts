...
import { newMarkdownState, renderMarkdownLine, type MarkdownState } from "./markdown";
import { visibleWidth } from "../text/text-width";
import { ANSI_PALETTE, rainbowText, rgbTo256, roleCode, type ColorRole, type Palette, type Rgb } from "../theme/theme";

/**
 * The pinned status region beneath the scrolling transcript, and the pieces it is built from.
 *
 * Archymedes stays a scrolling-transcript CLI — an append-only log is what makes a session reviewable
...
/**
 * A gradient-filled meter — Archymedes's answer to Bubbles' `progress` component. Bounded to `width`
...
/**
 * Aligned columns with a header rule — Archymedes's answer to Bubbles' `table` component, for the two
...
export class MarkdownStream {
  ...
  constructor(
     ...
     * in whatever file or program is reading. So `archymedes "..." > notes.md` gets the model's own text
 ...
 * the single-row case every one of Archymedes's split-pane screens actually builds with.
 *
...
function promptTitle(mode: string, workspace: string, glyphs: GlyphSet): string {
  return `${glyphs.star} archymedes ${glyphs.middot} ${mode} ${glyphs.middot} ${workspace}`;
}
 ...
 * session with a bare `archymedes ›` and no bar at all. This draws the same box by printing it and then
