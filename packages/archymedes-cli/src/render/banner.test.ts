...
import { renderBanner, renderTagline } from "./banner";
...
describe("banner", () => {
  it("keeps the wordmark column-aligned whatever the sky does around it", () => {
    ...
    for (const seed of [1, 42, 20260808, 999_331]) {
      ...
      // The letterform block is drawn into a fixed-width left margin, so each row's first filled
      // pixel lands in the same narrow band — a sheared banner would scatter these widely. The
      // small spread that remains is the glyph shapes themselves (an "A" apex is inset).
      const starts = rows.map((row) => row.indexOf("█"));
      expect(Math.max(...starts) - Math.min(...starts), `seed ${seed}`).toBeLessThanOrEqual(2);
    }
  });

  it("spells ARCHYMEDES and never exceeds the terminal width", () => {
    const width = 80;
  ...
  it("falls back to one line when the window is too narrow to draw block letters", () => {
    ...
    expect(narrow.split("\n")).toHaveLength(1);
    expect(narrow).toContain("ARCHYMEDES");
