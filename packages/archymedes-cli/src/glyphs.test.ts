...
describe("detecting what the terminal can draw", () => {
  it("obeys ARCHYMEDES_GLYPHS over every heuristic, in both directions", () => {
    expect(detectGlyphMode({ ARCHYMEDES_GLYPHS: "ascii", LANG: "en_US.UTF-8" }, "linux")).toBe("ascii");
    expect(detectGlyphMode({ ARCHYMEDES_GLYPHS: "unicode", TERM: "linux" }, "linux")).toBe("unicode");
  });
  ...
  it("resolves straight to a usable set", () => {
    expect(resolveGlyphs({ ARCHYMEDES_GLYPHS: "ascii" }, "linux")).toBe(ASCII_GLYPHS);
    expect(resolveGlyphs({ LANG: "en_GB.UTF-8" }, "linux")).toBe(UNICODE_GLYPHS);
    expect(glyphsFor("ascii")).toBe(ASCII_GLYPHS);
  });

  it("honours ARCHYMEDES_ASCII, the flag people already reach for, but not when it is switched off", () => {
    expect(detectGlyphMode({ ARCHYMEDES_ASCII: "1", LANG: "en_US.UTF-8" }, "linux")).toBe("ascii");
    expect(detectGlyphMode({ ARCHYMEDES_ASCII: "0", LANG: "en_US.UTF-8" }, "linux")).toBe("unicode");
  });
});
