...
/**
 * Drives the real Archymedes CLI binary under a pseudo-terminal.
 *
 * `child_process.spawn` with piped stdio is not a substitute: `archymedes.ts` refuses to run
...
export function spawnArchymedes(options: SpawnArchymedesOptions): ArchymedesProcess {
  const proc = pty.spawn(bunExecutable(), ["run", ARCHYMEDES_ENTRY, ...(options.args ?? [])], {
    name: "xterm-color",
