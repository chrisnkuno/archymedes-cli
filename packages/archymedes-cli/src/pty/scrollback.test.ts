 ...
 *
 * This is the one property no unit test can express, because it is not about what Archymedes prints — it
 * is about what the *terminal* does with what Archymedes printed. A terminal saves a scrolled-off line to
...
describe("scrollback, under a real pty", () => {
  ...
  beforeEach(async () => {
    ...
    cwd = await mkdtemp(path.join(os.tmpdir(), "archymedes-scroll-"));
    configDir = await mkdtemp(path.join(os.tmpdir(), "archymedes-scroll-config-"));
  });
