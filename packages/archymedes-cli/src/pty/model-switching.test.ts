...
beforeAll(async () => {
  ...
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-model-"));
});
afterAll(async () => { await stub.close(); });

/** A fresh config directory per test, so one test's persisted model cannot leak into another. */
async function boot() {
  configDir = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-modelcfg-"));
...
describe("switching models under a real pty", () => {
  it("sends the earlier requests and answers to the newly selected model", async () => {
    ...
    await p.waitFor(/I will remember cobalt.*completed.*›/s, { timeoutMs: 20_000, since: firstTurnAt });
  ...
  it("remembers the switch, so the next launch starts on it", async () => {
    ...
    expect(saved).toMatchObject({ ARCHYMEDES_PROVIDER: "anthropic", ANTHROPIC_MODEL: "claude-haiku-4-5" });

    const second = spawnArchymedes({ cwd, args: ["--currency", "USD"], env: {
      ANTHROPIC_API_KEY: "sk-ant-test", ANTHROPIC_BASE_URL: stub.url,
      ARCHYMEDES_CONFIG_DIR: configDir, ARCHYMEDES_FX_OFFLINE: "true", TZ: "UTC",
    }});
  ...
  it("reaches the API keys from a row in the picker", async () => {
    ...
    expect(seen.slice(jumped)).toContain("Archymedes settings");
    p.kill();
  }, 60_000);
