...
/**
 * End-to-end tests that drive the real `archymedes` binary under a real pseudo-terminal.
 *
 ...
 * Ctrl+C reaches the process — see the comment on `bindSigint` in `archymedes.ts`), SIGWINCH fires on
 * resize, and multi-byte input arrives as raw bytes that have to survive echo. `child_process.spawn`
 * with piped stdio cannot stand in for any of this — `archymedes.ts` refuses to run interactively without
...
describe("archymedes CLI under a real pty", () => {
  ...
  beforeEach(async () => {
    ...
    cwd = await mkdtemp(path.join(os.tmpdir(), "archymedes-pty-"));
    ...
    configDir = await mkdtemp(path.join(os.tmpdir(), "archymedes-pty-config-"));
  });
  ...
  describe("managed application previews", () => {
    it("keeps a verified URL reachable after the tool call and tears it down when the CLI exits", async () => {
      ...
      await p.waitFor(/Archymedes wants to/, { timeoutMs: 15_000, since: before });
      p.writeLine("y");
  ...
  describe("provider failure messages", () => {
    ...
    it("turns an authentication failure into a concrete settings action without retrying", async () => {
      ...
      expect(turn).toContain("archymedes --doctor");
  ...
  describe("an approval prompt interrupted by Ctrl+C", () => {
    ...
    // step the runtime loops over, so without the fix in `createApprovalPrompt` (archymedes.ts) — racing
    ...
    it("does not hang — the turn is denied and the prompt comes back", async () => {
      ...
      stub.enqueue({ kind: "tool_call", toolName: "run_command", input: { command: "echo hello-from-archymedes-test" } });
      const before = p.output().length;
      p.writeLine("run echo hello-from-archymedes-test");
      await p.waitFor(/Archymedes wants to/, { timeoutMs: 15_000, since: before });
  ...
  describe("the pinned footer", () => {
    // Archymedes's transcript has always been an append-only scroll; this is the one thing that changed
    ...
    // wiring into `archymedes.ts` still wrong.
    //
  ...
  describe("mode, model and thread switches", () => {
    ...
    // same coordinator (ArchymedesSessionDaemon) — the exact rewrite this suite exists to verify: none
  ...
  describe("undo, by scope", () => {
    async function initGitRepo(): Promise<void> {
      ...
      await run(["init", "-q"]);
      await run(["config", "user.email", "archymedes@test"]);
      await run(["config", "user.name", "Archymedes"]);
      await run(["add", "-A"]);
