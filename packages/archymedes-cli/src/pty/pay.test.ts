...
describe("paying from the prompt", () => {
  ...
  beforeEach(async () => {
    ...
    cwd = await mkdtemp(path.join(os.tmpdir(), "archymedes-pty-pay-"));
    configDir = await mkdtemp(path.join(os.tmpdir(), "archymedes-pty-pay-config-"));
  });
  ...
  function boot(env: Record<string, string> = {}): ArchymedesProcess {
    proc = spawnArchymedes({
      ...
      env: {
        ...
        ARCHYMEDES_BILLING_URL: billing.url,
        ARCHYMEDES_BILLING_KEY: "sk-billing-test",
        TZ: "UTC",
        ...env,
      },
    });
  ...
  it("creates nothing until the person confirms, then creates exactly one payment", async () => {
    ...
    expect(billing.creations()[0].idempotencyKey).toMatch(/^archymedes-topup-/);
  ...
  it("says which settings are missing instead of half-working", async () => {
    const p = boot({ ARCHYMEDES_BILLING_URL: "", ARCHYMEDES_BILLING_KEY: "" });
    await p.waitFor(PROMPT, { timeoutMs: 60_000 });
    const since = p.output().length;
    p.writeLine("/pay 5000");
    await p.waitFor(/ARCHYMEDES_BILLING_URL/, { since, timeoutMs: 30_000 });
    expect(p.output().slice(since)).toContain("ARCHYMEDES_BILLING_KEY");
    expect(billing.creations()).toHaveLength(0);
  });

  it("sets and persists a local balance without calling the balance endpoint", async () => {
    const p = boot({
      ARCHYMEDES_BILLING_URL: "",
    ...
    expect(JSON.parse(await readFile(path.join(configDir, "settings.json"), "utf8"))).toMatchObject({
      ARCHYMEDES_ACCOUNT_BALANCE_RWF: "5000",
    });
