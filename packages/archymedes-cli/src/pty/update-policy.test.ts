 ...
 *
 * The unit tests decide when Archymedes may update itself; this proves the decision is reachable from the
...
describe("choosing how Archymedes updates itself", () => {
  ...
  beforeEach(async () => {
    ...
    cwd = await mkdtemp(path.join(os.tmpdir(), "archymedes-pty-update-"));
    configDir = await mkdtemp(path.join(os.tmpdir(), "archymedes-pty-update-config-"));
  });
  ...
  it("remembers the policy in a file the next launch reads", async () => {
    ...
    expect(settings.ARCHYMEDES_AUTO_UPDATE).toBe("install");
  });
  ...
  it("says what the choices are rather than guessing at a word it does not know", async () => {
    ...
    await expect(readFile(path.join(configDir, "settings.json"), "utf8").catch(() => "{}"))
      .resolves.not.toContain("ARCHYMEDES_AUTO_UPDATE");
  });
});
