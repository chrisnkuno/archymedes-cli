...
beforeAll(async () => {
  ...
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-keys-"));
  configDir = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-keyscfg-"));
});
afterAll(async () => { await stub.close(); });

function boot() {
  return spawnArchymedes({ cwd, args: ["--currency", "USD"], env: {
