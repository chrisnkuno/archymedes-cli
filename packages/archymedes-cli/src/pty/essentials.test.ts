 ...
 *
 * Every command Archymedes has is discoverable — through `/help`, the palette, the guide. Discoverable is
...
describe("what a first session points at", () => {
  ...
  beforeEach(async () => {
    ...
    cwd = await mkdtemp(path.join(os.tmpdir(), "archymedes-pty-first-"));
    configDir = await mkdtemp(path.join(os.tmpdir(), "archymedes-pty-first-config-"));
  });
