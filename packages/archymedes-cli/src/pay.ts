import {
  ...
  type Checkout,
  type Payment,
} from "@archymedes/core/cli/billing";

/**
 * `/pay` — the command grammar and everything it prints.
 *
 * Split from the flow in archymedes.ts on purpose. This file is where the wording of a money screen
...
export { CRITICAL_BALANCE_RWF } from "@archymedes/core/cli/billing";
...
export type BalanceWatchOptions = {
  /** A simple floor for a new or nearly idle session, configurable through ARCHYMEDES_LOW_BALANCE_RWF. */
  ...
  observe(
    ...
    if (kind === "empty") {
      return {
        kind,
        lines: [
          "Balance watch: no Archymedes credit remains.",
          "Archymedes will not top up automatically. Add credit when you choose with /pay 5000.",
        ],
      };
    }
    if (kind === "critical") {
      return {
        ...
        lines: [
          ...
          "Archymedes will check a task's estimate before sending it. Nothing will top up automatically; use /pay when you choose.",
        ],
      };
    }
    ...
    if (kind === "rapid") {
      ...
      return {
        ...
        lines: [
          ...
          "You are still in control: Archymedes will not top up automatically. Use /cost to review it, /slow to reduce the pace, or /pay when you choose.",
        ],
      };
    }
    return {
      ...
      lines: [
        ...
        "Archymedes will not top up automatically. Use /cost to review spending, /slow to reduce the pace, or /pay when you choose.",
      ],
    };
  }
}
...
export function renderTopUpQuote(amountRwf: number, balance?: Balance): string[] {
  ...
  lines.push("You will pay on Circuit Pay's own page — Archymedes never sees your card or PIN.");
  return lines;
}
...
export const BILLING_NOT_CONFIGURED = [
  ...
  "Set ARCHYMEDES_BILLING_URL and ARCHYMEDES_BILLING_KEY in /settings, then run /pay again.",
];
