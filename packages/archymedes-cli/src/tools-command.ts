import type { AgentTool } from "@archymedes/core";
 ...
 * `.archymedes/skills`, a plugin someone committed, an MCP server declared in `.archymedes/mcp.json` — all of
 ...
 * script can block calls, and an unexplained refusal is indistinguishable from a Archymedes bug unless
...
export function renderTools(view: ToolsView, style: ToolsStyle = plain): string {
  ...
  if (groups.size === 0 && (view.emptyProviders?.length ?? 0) === 0) {
    ...
    lines.push(`  ${style.dim("Add one under .archymedes/skills, .archymedes/plugins, or declare a server in .archymedes/mcp.json.")}`);
  } else {
    for (const [label, names] of groups) {
      lines.push("");
      // Yellow, matching how the CLI already marks anything not local-and-ordinary: these are tools
      // Archymedes did not ship, every one of them approval-gated on every call.
  ...
  if (preToolUse.length > 0 || postToolUse.length > 0) {
    ...
    // about when a tool call is refused for a reason Archymedes itself never decided.
