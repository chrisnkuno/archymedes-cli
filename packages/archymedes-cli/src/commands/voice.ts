...
export async function startRecording(
  ...
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-voice-"));
  const file = path.join(directory, "prompt.wav");
  const executable = environment.ARCHYMEDES_FFMPEG_PATH?.trim() || "ffmpeg";
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.once("error", (error) => { clearTimeout(ready); reject(new Error(`Could not start microphone capture. Install ffmpeg or set ARCHYMEDES_FFMPEG_PATH. ${error.message}`)); });
...
export async function transcribeAudio(
  ...
  if (!apiKey) throw new Error("Voice input needs OPENAI_API_KEY. Add it in archymedes settings or set it in your environment.");
