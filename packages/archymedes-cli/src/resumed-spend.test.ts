...
import { EventJournal } from "@archymedes/core/cli/protocol";
import { fromUnits, tokenPrices, toUnits, type FxRate } from "@archymedes/core/money";
...
describe("spend rebuilt from a session's journal", () => {
  it("reads back every model call, in the order they happened", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-spend-"));
    try {
  ...
  it("refuses a journal whose chain has been tampered with, rather than pricing it", async () => {
    ...
    try {
      ...
      const file = path.join(root, ".archymedes", "events", "session_1.jsonl");
