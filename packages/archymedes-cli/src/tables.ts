import type { JobStatus, JobSummary } from "@archymedes/core/cli/jobs";
...
/**
 * Which columns each of Archymedes's tables has — and nothing about how a table is drawn.
 *
...
export function buildModelTable(
  ...
  const { paint } = options;
  const notes = [
    // A provider Archymedes could use but has no key for is the most common reason this list is short, and
    // naming the variable is the difference between a short list and an actionable one.
    ...catalog.unconfigured.map((entry) => `${paint.dim(entry.label)} ${paint.yellow(`— archymedes settings, or set ${entry.missing.join(" and ")}`)}`),
