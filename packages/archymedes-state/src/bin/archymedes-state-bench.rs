...
use archymedes_state::{integrity_for_session, read_journal, read_session, SearchOptions, StateIndex};
...
/// a corpus without them measures a workspace shape Archymedes never actually writes.
fn write_corpus(
    root: &Path,
    sessions: usize,
    messages: usize,
    ...
    let snapshots = root.join(".archymedes/sessions");
    let journals = root.join(".archymedes/events");
...
fn time_sources(root: &Path) -> (Duration, Duration) {
    ...
    for entry in fs::read_dir(root.join(".archymedes/sessions")).unwrap().flatten() {
        let path = entry.path();
    ...
    for entry in fs::read_dir(root.join(".archymedes/events")).unwrap().flatten() {
        let path = entry.path();
        let session_id = path.file_stem().unwrap().to_string_lossy().into_owned();
        let started = Instant::now();
