...
13. **Concurrent-process safety.** Schema initialization and replay writers serialize before
    mutation; a second Archymedes process cannot delete, split, or partially replace the active index.

## Benchmark contract

`archymedes-state-bench` generates an isolated synthetic corpus and reports indexing throughput, SQLite
...
The corpus has to be shaped like a real workspace or the benchmark prices work Archymedes never does:
...
is meant to represent Archymedes leaves both at their defaults.

```sh
cargo run --release --features benchmark --manifest-path packages/archymedes-state/Cargo.toml \
  --bin archymedes-state-bench -- --sessions 500 --messages 40 --queries 200
```
