...
use archymedes_state::{handle_request, read_journal, Request, SearchOptions, StateIndex};
...
fn write_journal(root: &Path, session_id: &str, marker: &str) -> Vec<Value> {
    let directory = root.join(".archymedes/events");
...
fn write_snapshot_messages(root: &Path, session_id: &str, title: &str, messages: Vec<Value>) {
    let directory = root.join(".archymedes/sessions");
    fs::create_dir_all(&directory).unwrap();
...
fn invariant_canonical_sources_are_byte_identical_after_replay() {
    ...
    let journal = directory.path().join(".archymedes/events/session-a.jsonl");
    let snapshot = directory.path().join(".archymedes/sessions/session-a.json");
...
fn invariant_schema_mismatch_discards_only_the_projection() {
    ...
    assert!(directory
        .path()
        .join(".archymedes/events/session-a.jsonl")
        .exists());
    assert!(directory
        .path()
        .join(".archymedes/sessions/session-a.json")
        .exists());
}
...
fn invariant_verified_events_keep_their_digest_and_field_order() {
    ...
    let path = directory.path().join(".archymedes/events/session-a.jsonl");
...
fn invariant_batched_rebuild_isolates_a_failure_and_stays_idempotent() {
    ...
    fs::write(
        directory.path().join(".archymedes/sessions/batch-40.json"),
        b"{ not json",
    )
    .unwrap();
...
fn invariant_reindexing_replaces_rather_than_accumulates_documents() {
    ...
    fs::remove_file(directory.path().join(".archymedes/sessions/session-a.json")).unwrap();
    fs::remove_file(directory.path().join(".archymedes/events/session-a.jsonl")).unwrap();
    index.rebuild_all().unwrap();
    assert!(index.sessions(20).unwrap().is_empty());
    assert!(index
        .search("PaymentIntent", SearchOptions::default())
...
fn behavior_empty_sources_index_without_documents_or_panics() {
    ...
    fs::create_dir_all(directory.path().join(".archymedes/events")).unwrap();
    fs::write(directory.path().join(".archymedes/events/empty.jsonl"), "").unwrap();
