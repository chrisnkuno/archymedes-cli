...
Each release compiles `archymedes-state` for one target and stages an intentionally tiny npm package:
...
added to `archymedes-cli` as optional dependencies and the CLI published. Keeping those two
release phases separate prevents a CLI version from referring to native packages that are not yet
available in the registry.

The TypeScript bridge also supports `ARCHYMEDES_STATE_BINARY` for development and unsupported packagers.
If no compatible binary exists, Archymedes can retain its TypeScript projection instead of failing to
