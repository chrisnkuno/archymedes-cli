...
`archymedes-state` is Archymedes's local, rebuildable history index. It verifies the existing session snapshots
...
`.archymedes/security-brain/brain-v1.sqlite3`; it never mixes security research with conversation history
...
This first milestone is deliberately read-only with respect to canonical Archymedes state. It writes only
`.archymedes/state/index-v1.sqlite3`, which can be deleted and rebuilt at any time. Session snapshots,
...
{"id":"5","protocolVersion":1,"method":"brain.rebuild","params":{"sourceRoot":"/installed/defender-knowledge","dataRoot":"/workspace/.archymedes/security-brain"}}
{"id":"6","protocolVersion":1,"method":"brain.search","params":{"sourceRoot":"/installed/defender-knowledge","dataRoot":"/workspace/.archymedes/security-brain","query":"post quantum migration","limit":4,"now":"2026-08-24"}}
```
...
turn counts never attribute Archymedes's own nudges to the user.
...
```sh
cargo test --manifest-path packages/archymedes-state/Cargo.toml
cargo fmt --manifest-path packages/archymedes-state/Cargo.toml -- --check
cargo clippy --manifest-path packages/archymedes-state/Cargo.toml --all-targets -- -D warnings
cargo run --manifest-path packages/archymedes-state/Cargo.toml
cargo run --release --features benchmark --manifest-path packages/archymedes-state/Cargo.toml --bin archymedes-state-bench -- --sessions 500 --messages 40 --queries 200
cargo run --release --features benchmark --manifest-path packages/archymedes-state/Cargo.toml --bin archymedes-defender-brain-bench -- --records 10000 --queries 2000
```
...
`.archymedes/security-brain/candidates/`. It cannot modify the active corpus: a human must verify the
