...
describe("oversized tool results", () => {
  ...
  function storeSpy(behaviour: "ok" | "throws" = "ok") {
    ...
    const store: ToolResultArtifactStore = {
      async put(input) {
        ...
        return { path: ".archymedes/artifacts/run_command-0123456789ab.txt", bytes: Buffer.byteLength(input.content), lines: input.content.split("\n").length, elided: false };
      },
    };
    return { store, puts };
  }
  ...
  it("writes the whole result to an artifact and charges the transcript only for the excerpt", async () => {
    ...
    expect(toolMessage?.content).toContain(".archymedes/artifacts/run_command-0123456789ab.txt");
  ...
  it("tells the front end where the full output went", async () => {
    ...
    expect(event).toMatchObject({ type: "tool_result", artifact: { path: ".archymedes/artifacts/run_command-0123456789ab.txt", elided: false } });
  });
...
describe("what a tool result costs the transcript", () => {
    ...
    return {
      ...
      store: {
        async put(input: { toolName: string; content: string }) {
          ...
          return { path: `.archymedes/artifacts/${input.toolName}-abc123456789.txt`, bytes: input.content.length, lines: input.content.split("\n").length, elided: false };
        },
  ...
  it("still evicts when the result is genuinely large, and spends a fraction of the budget on the excerpt", async () => {
    ...
    expect(toolMessage?.content).toContain(".archymedes/artifacts/run_command-abc123456789.txt");
