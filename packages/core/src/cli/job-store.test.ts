...
beforeEach(async () => {
  root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-jobs-"));
});
...
describe("persisting across separate calls", () => {
  ...
  it("writes a real file at .archymedes/jobs.json", async () => {
...
describe("the whole lease lifecycle through the file", () => {
  ...
  it("carries an approval through requestJobApproval / resolveJobApproval / consumeJobApproval", async () => {
    ...
    const request = {
      ...
      actionDigest: "digest-apply", scopeKey: "archymedes-approval-v1:digest-apply", policyVersion: "archymedes-approval-v1",
