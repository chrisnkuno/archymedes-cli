...
describe("every real Archymedes tool", () => {
  let root: string;
  const tools = () => createArchymedesTools({ workspace: new LocalWorkspace(root), todos: new TodoList(), fetchImpl: (async () => new Response("ok")) as typeof fetch });

  it("declares a schema this validator supports", async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-schema-"));
    try {
  ...
  it("holds the structural invariants the runtime relies on", async () => {
    ...
    try {
      ...
      for (const tool of await tools()) {
        ...
        expect(Object.values(ARCHYMEDES_CAPABILITY_IDS)).toContain(tool.capabilityId);
...
describe("the tool contract is pinned", () => {
  it("matches the golden surface, exactly and in order", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-golden-"));
    try {
      ...
      const actual = (await createArchymedesTools({
...
const ARCHYMEDES_CAPABILITY_IDS = {
