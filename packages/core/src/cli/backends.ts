...
/**
 * Where Archymedes's files actually live.
 *
 ...
 * twice.
 */
export interface ArchymedesWorkspace {
   ...
   * is, so a skill's arguments must be POSIX-quoted for an E2B session even when Archymedes itself is on
   ...
   *
   * Exists for exactly one caller: loading Archymedes's own configuration out of `.archymedes` (skills, hooks,
   * plugins, MCP servers). `.archymedes` sits in `ignoredDirectories` so the *agent's* searches are not
   * cluttered by Archymedes's own bookkeeping, which is right — but that list was never meant to hide
   * configuration from Archymedes itself, and routing config discovery through `list`/`glob` means it
   * silently finds nothing on every backend.
   *
   * Deliberately narrow: files only (every config file Archymedes looks for is a file), bounded, and
...
/**
 * Ceiling on a config scan. `.archymedes` holds manifests and small scripts; anything approaching this
 ...
 */
abstract class SandboxWorkspace implements ArchymedesWorkspace {
