...
function scripted(turns: Array<Partial<AgentModelTurn>>): AgentTurnProvider & { calls: number } {
  ...
  return {
    ...
    async complete() {
      ...
      index += 1;
      return { responseId: `r${index}`, model: "archymedes-test", finishReason: "stop", content: "Done.", toolCalls: [], usage, ...turn } as AgentModelTurn;
    },
  };
}
...
describe("a whole agent turn, recorded and replayed", () => {
  let root: string;
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-cassette-"));
  ...
  const agentWith = (model: AgentTurnProvider) => new ArchymedesAgent({
  ...
  it("produces the golden event stream, in order", async () => {
    ...
    const events: string[] = [];
    const replayed = new ArchymedesAgent({
