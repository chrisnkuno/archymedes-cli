...
const MCP_CONFIG_PATH = ".archymedes/mcp.json";
...
/**
 * `.archymedes/mcp.json`: `{ "servers": [...] }`, for MCP servers wanted every session rather than only
 * inside a plugin. A missing file is zero servers, not an error.
 *
 * Read through the workspace like every other `.archymedes` manifest, so the declaration travels with the
 * repository. Note this configures where to *find* servers, not where they run: an MCP server is a
 * process Archymedes spawns on the machine Archymedes itself is running on, which is true regardless of whether
 ...
 * below is what turns its tools into something `createArchymedesTools` can merge in.
 */
export class McpConnection {
   ...
   *
   * `createArchymedesTools` runs on every turn, so this was a `tools/list` JSON-RPC round trip per turn
...
/** Exposes one MCP server's tools as a `ToolProvider`. Schemas outside Archymedes's supported subset (see tool-schema.ts) fail loudly at registration rather than reaching the model unvalidated. */
