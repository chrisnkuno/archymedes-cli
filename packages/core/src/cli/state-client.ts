...
export const ARCHYMEDES_STATE_PROTOCOL_VERSION = 1 as const;
...
export class ArchymedesStateError extends Error {
  constructor(readonly code: string, message: string) { super(message); this.name = "ArchymedesStateError"; }
}
...
const PLATFORM_PACKAGES: Readonly<Record<string, string>> = {
  "darwin-arm64": "@archymedes/state-darwin-arm64",
  "darwin-x64": "@archymedes/state-darwin-x64",
  "win32-arm64": "@archymedes/state-win32-arm64",
  "win32-x64": "@archymedes/state-win32-x64",
  "linux-arm64-gnu": "@archymedes/state-linux-arm64-gnu",
  "linux-arm64-musl": "@archymedes/state-linux-arm64-musl",
  "linux-x64-gnu": "@archymedes/state-linux-x64-gnu",
  "linux-x64-musl": "@archymedes/state-linux-x64-musl",
};
...
export async function resolveArchymedesStateBinary(environment: NodeJS.ProcessEnv = process.env): Promise<string | null> {
  if (environment.ARCHYMEDES_STATE_BINARY) {
    const explicit = path.resolve(environment.ARCHYMEDES_STATE_BINARY);
    if (!(await executable(explicit))) throw new Error(`ARCHYMEDES_STATE_BINARY is not executable: ${explicit}`);
    return explicit;
  }
  ...
  if (packageName) {
    try {
      ...
      const candidate = path.join(path.dirname(manifest), "bin", process.platform === "win32" ? "archymedes-state.exe" : "archymedes-state");
  ...
  const filename = process.platform === "win32" ? "archymedes-state.exe" : "archymedes-state";
  for (const profile of ["release", "debug"]) {
    const candidate = path.resolve(process.cwd(), "packages", "archymedes-state", "target", profile, filename);
    if (await executable(candidate)) return candidate;
  }
  return null;
}
...
export type ArchymedesStateClientOptions = {
...
export class ArchymedesStateClient {
  ...
  private constructor(options: ArchymedesStateClientOptions) {
    ...
    this.child.once("exit", (code, signal) => {
      ...
      this.failAll(new Error(`archymedes-state exited (${signal ?? code ?? "unknown"})${detail ? `: ${detail}` : ""}`));
    });
  }

  static async connect(options: ArchymedesStateClientOptions): Promise<ArchymedesStateClient> {
    const client = new ArchymedesStateClient(options);
    try {
  ...
  private receive(line: string): void {
    let parsed: unknown;
    try { parsed = JSON.parse(line); } catch { this.failAll(new Error("archymedes-state emitted malformed JSON")); return; }
    if (!isStateResponse(parsed)) { this.failAll(new Error("archymedes-state emitted a malformed protocol response")); return; }
    ...
    if (response.protocolVersion !== ARCHYMEDES_STATE_PROTOCOL_VERSION) {
      pending.reject(new ArchymedesStateError("unsupported_protocol", `Expected archymedes-state protocol ${ARCHYMEDES_STATE_PROTOCOL_VERSION}, received ${response.protocolVersion}`));
    } else if (!response.ok) {
      pending.reject(new ArchymedesStateError(response.error?.code ?? "state_error", response.error?.message ?? "archymedes-state request failed"));
  ...
  private call<T>(method: string, params: Record<string, unknown>): Promise<T> {
    if (this.closed) return Promise.reject(new Error("archymedes-state client is closed"));
    const id = String(++this.nextId);
    return new Promise<T>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pending.delete(id);
        reject(new ArchymedesStateError("timeout", `archymedes-state ${method} timed out after ${this.timeoutMs}ms`));
      }, this.timeoutMs);
      this.pending.set(id, { resolve: resolve as (value: unknown) => void, reject, timer });
      this.child.stdin.write(`${JSON.stringify({ id, protocolVersion: ARCHYMEDES_STATE_PROTOCOL_VERSION, method, params })}\n`, (error) => {
        if (!error) return;
  ...
  async close(): Promise<void> {
    ...
    this.failAll(new Error("archymedes-state client closed"));
  }
}

/** Missing native support is a visible capability downgrade, never a CLI startup failure. */
export async function tryConnectArchymedesState(options: Omit<ArchymedesStateClientOptions, "binary"> = {}): Promise<ArchymedesStateClient | null> {
  ...
  // the explicit portable fallback immediately when Archymedes itself is launched under Bun. Published
  ...
  const binary = await resolveArchymedesStateBinary(options.environment);
  return binary ? ArchymedesStateClient.connect({ ...options, binary }) : null;
}
