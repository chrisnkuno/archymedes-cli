...
describe("what counts as an amount", () => {
  ...
  it("gives every attempt its own idempotency key", () => {
    ...
    expect([...keys].every((key) => key.length <= 64 && key.startsWith("archymedes-topup-"))).toBe(true);
  });
});
...
describe("the Circuit Pay adapter", () => {
  ...
  it("never leaks the key into an error a user will paste somewhere", async () => {
    ...
    await expect(gateway.getBalance()).rejects.toThrow(/ARCHYMEDES_BILLING_KEY/);
  ...
  it("is absent, not half-configured, when the environment says nothing", () => {
    ...
    expect(billingFromEnvironment({ ARCHYMEDES_BILLING_URL: "https://pay.example" })).toBeNull();
    expect(billingFromEnvironment({ ARCHYMEDES_BILLING_KEY: "sk" })).toBeNull();
    expect(billingFromEnvironment({ ARCHYMEDES_BILLING_URL: "https://pay.example", ARCHYMEDES_BILLING_KEY: "sk" })).toBeInstanceOf(CircuitPayGateway);
  });
});
