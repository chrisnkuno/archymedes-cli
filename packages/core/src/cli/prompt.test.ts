...
beforeEach(async () => {
  root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-prompt-"));
});
...
describe("collectProjectContext", () => {
  it("finds an instructions file and prefers ARCHYMEDES.md when several exist", async () => {
    await fs.writeFile(path.join(root, "ARCHYMEDES.md"), "Use tabs.");
    await fs.writeFile(path.join(root, "AGENTS.md"), "Use spaces.");
    const context = await collectProjectContext(root);
    expect(context.instructionsFile).toBe("ARCHYMEDES.md");
  ...
  it("truncates an oversized instructions file rather than sending it whole", async () => {
    await fs.writeFile(path.join(root, "ARCHYMEDES.md"), "x".repeat(100));
  ...
  it("reads the current git branch from a real checkout", async () => {
    await fs.mkdir(path.join(root, ".git"));
    await fs.writeFile(path.join(root, ".git", "HEAD"), "ref: refs/heads/archymedes-cli-packages\n");
    const context = await collectProjectContext(root);
    expect(context.gitBranch).toBe("archymedes-cli-packages");
  });
...
describe("buildArchymedesSystemPrompt", () => {
  ...
  it("tells the model plainly when it is working on the user's real machine", () => {
    const prompt = buildArchymedesSystemPrompt(context, "build", ["read_file"]);
  ...
  it("tells the model plainly when it is working in a disposable remote sandbox instead", () => {
    ...
    const prompt = buildArchymedesSystemPrompt(context, "build", ["read_file"], workspace as never);
  ...
  it("asks for invariant-based tests rather than a single happy-path example", () => {
    ...
    const prompt = buildArchymedesSystemPrompt(context, "build", ["run_command"]);
  ...
  it("requires verified managed previews instead of trusting server startup text", () => {
    const prompt = buildArchymedesSystemPrompt(context, "build", ["run_command", "start_application", "application_status"]);
  ...
  it("tells the agent to batch its tool calls, which is the largest single cost in a turn", () => {
    ...
    const prompt = buildArchymedesSystemPrompt(context, "build", ["run_command", "todo_write"]);
  ...
  it("states the exact behavioural boundary for each mode", () => {
    expect(buildArchymedesSystemPrompt(context, "plan", [])).toContain("cannot write files or run commands");
    expect(buildArchymedesSystemPrompt(context, "build", [])).toContain("approved by the user before it runs");
    expect(buildArchymedesSystemPrompt(context, "auto", [])).toContain("run without individual approval");
    expect(buildArchymedesSystemPrompt(context, "defender", [])).toContain("every effectful call is approved by the user");
  });

  it("permits contained credential workflows without permitting disclosure", () => {
    const prompt = buildArchymedesSystemPrompt(context, "auto", ["read_file", "write_file"]);
  ...
  it("indexes every security playbook in defender mode, and none of them anywhere else", () => {
    const defenderPrompt = buildArchymedesSystemPrompt(context, "defender", ["query_defensive_brain", "read_playbook"]);
    ...
    const buildPrompt = buildArchymedesSystemPrompt(context, "build", []);
  ...
  it("names the maintainers' file and puts their instructions ahead of Archymedes's own habits", () => {
    const prompt = buildArchymedesSystemPrompt(
      { root: "/repo", instructions: "Always use tabs.", instructionsFile: "ARCHYMEDES.md", layout: [], packageScripts: [], gitBranch: null },
      "build", [],
    );
    expect(prompt).toContain("Project instructions from ARCHYMEDES.md");
    expect(prompt).toContain("Always use tabs.");
  });

  it("lists the tools actually offered, which changes with mode", () => {
    const prompt = buildArchymedesSystemPrompt(context, "plan", ["read_file", "grep_files"]);
...
describe("the environment section", () => {
  ...
  it("puts what is installed in front of the model before it writes a command", async () => {
    const prompt = buildArchymedesSystemPrompt(await collectProjectContext(root), "build", ["run_command"], undefined, report);
