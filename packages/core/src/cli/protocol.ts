...
/**
 * Archymedes's narrow waist between the kernel, persistence and every present or future client.
 *
 ...
 */
export const ARCHYMEDES_PROTOCOL_VERSION = 1 as const;
...
export function assertTurnTransition(from: TurnStatus, to: TurnStatus): void {
  if (!TRANSITIONS[from].includes(to)) throw new Error(`Illegal Archymedes turn transition: ${from} -> ${to}`);
}
...
export type ArchymedesProtocolPayload =
...
export type ArchymedesEventEnvelope = {
  protocolVersion: typeof ARCHYMEDES_PROTOCOL_VERSION;
  sequence: number;
  sessionId: string;
  timestamp: string;
  previousHash: string;
  payload: ArchymedesProtocolPayload;
  hash: string;
};

function hashEnvelope(envelope: Omit<ArchymedesEventEnvelope, "hash">): string {
...
export function eventJournalPath(root: string, sessionId: string): string {
  ...
  return path.join(root, ".archymedes", "events", `${sessionId}.jsonl`);
}
 ...
 */
export async function readEventJournal(root: string, sessionId: string): Promise<ArchymedesEventEnvelope[]> {
  return (await readJournalFile(root, sessionId)).events;
}

async function readJournalFile(root: string, sessionId: string): Promise<{ events: ArchymedesEventEnvelope[]; text: string; completeText: string }> {
  ...
  const events = completeText.split("\n").filter(Boolean).map((line) => JSON.parse(line) as ArchymedesEventEnvelope);
  let previousHash = JOURNAL_GENESIS_HASH;
  for (let index = 0; index < events.length; index += 1) {
    const event = events[index];
    if (event.protocolVersion !== ARCHYMEDES_PROTOCOL_VERSION) throw new Error(`Unsupported Archymedes protocol version ${event.protocolVersion}`);
    if (event.sessionId !== sessionId || event.sequence !== index + 1 || event.previousHash !== previousHash) {
      throw new Error(`Invalid Archymedes event chain at sequence ${event.sequence}`);
    }
    const { hash, ...withoutHash } = event;
    if (hash !== hashEnvelope(withoutHash)) throw new Error(`Archymedes event integrity check failed at sequence ${event.sequence}`);
    previousHash = hash;
  }
