...
import { createArchymedesTools, TodoList } from "./tools";
 ...
 * - Archymedes itself, which can read the failing targets and their remediation and act on them.
 *
...
async function fixedRequestTokens(root: string, mode: "build" | "plan" | "defender"): Promise<number> {
  ...
  const tools = await createArchymedesTools({
    ...
    // the fixed-cost report look roughly 200 tokens cheaper than the request Archymedes actually sends.
  ...
  const prompt = buildArchymedesSystemPrompt(context, mode, scoped.map((tool) => tool.name));
...
export const OPTIMIZATION_TARGETS: readonly OptimizationTarget[] = [
  ...
  },
  {
    ...
    evidence: "archymedes-cli/defender-playbooks.ts, read_playbook in archymedes-cli/tools.ts",
    remediation:
  ...
  },
  {
    ...
    evidence: "archymedes-cli/prompt.ts, providers/anthropic-agent.ts withCacheBreakpoints",
    remediation:
  ...
  },
  {
    ...
    evidence: "archymedes-cli/session.ts estimateMessageTokens / compactionUrgency",
    remediation:
  ...
  },
  {
    ...
    evidence: "archymedes-cli/session.ts, providers/model-capabilities.ts",
    remediation:
      "Either the estimator regressed or the session stopped deriving its context limit from the model. Check `ArchymedesAgent`'s constructor still merges provider capabilities ahead of the defaults.",
  ...
  },
  {
    ...
    evidence: "archymedes-cli/session.ts recentToKeep, planCompaction",
    remediation:
    ...
    measure: async () => {
      ...
      const messages: Parameters<typeof planCompaction>[0][number][] = [
        { role: "system", content: "You are Archymedes." },
  ...
  },
  {
    ...
    evidence: "agent-runtime.ts agentMessagePromptParts, archymedes-cli/session.ts estimateMessageTokens",
    remediation:
  ...
  },
  {
    ...
    evidence: "archymedes-cli/tools.ts deep_research successful-synthesis response",
    remediation:
  ...
  },
  {
    ...
    evidence: "archymedes-cli/cost.ts predictAgentUsage defender profile",
    remediation:
  ...
  },
  {
    ...
    layer: "state",
    what: "Median search query served by the archymedes-state index",
    metric: "milliseconds",
    ...
    evidence: "packages/archymedes-state/src/index.rs, `bun run bench:state`",
    remediation:
  ...
  },
  {
    ...
    evidence: "packages/archymedes-state/src/index.rs rebuild_all / write_batch",
    remediation:
  ...
  },
  {
    ...
    evidence: "packages/archymedes-state/src/bin/archymedes-state-bench.rs · `sources.shareOfIndex`",
    remediation:
  ...
  },
  {
    ...
    evidence: "archymedes-cli/workspace.ts walkWorkspace",
    remediation:
  ...
  },
  {
    ...
    evidence: "archymedes-cli/workspace.ts grepWorkspace",
    remediation:
  ...
  },
  {
    ...
    metric: "milliseconds",
    budget: { max: 200 },
    evidence: "packages/archymedes-cli/src/archymedes.ts, tooling/build/cli-bundle.ts",
    remediation:
      "Two mechanisms, and both are load-bearing. The bundler must keep `splitting: true` with `.mjs` chunk naming, or Bun hoists every dynamically imported subtree into the entry file and the provider SDKs execute on `archymedes --help` (entry 3.90 MB vs 0.94 MB; 0.17s vs 0.12s measured). And the launcher (dist/archymedes.mjs) must stay a separate tiny file that calls module.enableCompileCache() before *dynamically* importing the main bundle — a static import is hoisted above the call and silently disables it. Measure with: `bun run build:cli` then time `node dist/archymedes.mjs --help`.",
  },
];
