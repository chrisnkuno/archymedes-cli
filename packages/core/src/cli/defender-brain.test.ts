...
describe("DefenderBrain", () => {
  ...
  it("coalesces rebuilds and bounds caller-provided limits", async () => {
    ...
    const brain = new DefenderBrain(path.resolve(".archymedes/test-brain"), async () => client, [corpus]);
  ...
  it("returns a visible downgrade instead of throwing", async () => {
    const brain = new DefenderBrain(path.resolve(".archymedes/test-brain"), async () => null, [corpus]);
  ...
  it("falls back to the bundled corpus when native validation rejects a signed replica", async () => {
    const rejected = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-rejected-replica-"));
    ...
    try {
      const brain = new DefenderBrain(path.resolve(".archymedes/test-brain"), async () => client, [corpus], async () => rejected);
