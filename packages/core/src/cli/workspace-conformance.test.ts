...
import { DockerWorkspace, E2BWorkspace, LocalWorkspace, type ArchymedesWorkspace } from "./backends";
 ...
 *
 * `ArchymedesWorkspace` is the interface the tool layer is written against, and the model is shown the
...
type Backend = { name: string; workspace: ArchymedesWorkspace; seed(path: string, content: string): Promise<void>; cleanup(): Promise<void> };

async function localBackend(): Promise<Backend> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-conformance-"));
  return {
    name: "local",
...
describe.each(backends.map((factory) => [factory]))("workspace conformance", (factory) => {
  ...
  it("finds Archymedes's own config under .archymedes, which the agent-facing search deliberately hides", async () => {
    // The property that makes skills, hooks and plugins work off this machine at all. `.archymedes` is in
    // every backend's ignored-directory list so the *agent's* searches are not cluttered by Archymedes's
    ...
    const backend = await factory();
    try {
      await backend.seed(".archymedes/skills/greet/skill.json", "{}");
      await backend.seed("src/app.ts", "export const port = 3000;");

      const config = await backend.workspace.listConfigFiles(".archymedes/skills");
      expect(config).toContain(".archymedes/skills/greet/skill.json");

      // ...and the agent-facing search still does not see it, on every backend.
      const globbed = await backend.workspace.glob("**/*.json");
      expect(globbed).not.toContain(".archymedes/skills/greet/skill.json");
    } finally {
      await backend.cleanup();
    }
  });
  ...
  it("reports a missing config directory as nothing configured, never as a failure", async () => {
    const backend = await factory();
    try {
      await expect(backend.workspace.listConfigFiles(".archymedes/skills")).resolves.toEqual([]);
    } finally {
      await backend.cleanup();
    }
  });
