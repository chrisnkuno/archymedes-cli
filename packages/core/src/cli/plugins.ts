 ...
 * `.archymedes/plugins/<name>/plugin.json` manifest naming the MCP servers it wants connected, plus
 * whichever of `.archymedes/plugins/<name>/skills/<skill>/skill.json` and
 * `.archymedes/plugins/<name>/hooks/{pre,post}-tool-use/*` it happens to include — the same manifest
...
export const PLUGINS_DIRECTORY = ".archymedes/plugins";
...
/**
 * Every `.archymedes/plugins/<name>/plugin.json` in the workspace, plus that plugin's own directory as a
 ...
 */
export async function discoverPlugins(workspace: ArchymedesWorkspace): Promise<Array<{ manifest: PluginManifest; directory: string }>> {
  const files = await workspace.listConfigFiles(PLUGINS_DIRECTORY);
  // Exactly one level deep, mirroring skills: `.archymedes/plugins/<name>/plugin.json`.
