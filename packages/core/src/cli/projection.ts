...
import { eventJournalPath, readEventJournal, type ArchymedesEventEnvelope, type ArchymedesProtocolPayload } from "./protocol";
...
export type EventRow = { sessionId: string; sequence: number; timestamp: string; type: string; turnId: string | null; hash: string; payload: ArchymedesProtocolPayload };
...
function textOf(payload: ArchymedesProtocolPayload): Array<{ kind: string; text: string }> {
...
export class SessionProjection {
  ...
  static async open(root: string, options: { file?: string } = {}): Promise<SessionProjection> {
    const file = options.file ?? path.join(root, ".archymedes", "projection.db");
  ...
  eventsAfter(sessionId: string, cursor: number, limit = 500): EventRow[] {
    ...
    return rows.map((row) => ({
      ...
      payload: JSON.parse(row.payload as string) as ArchymedesProtocolPayload,
    }));
  }
