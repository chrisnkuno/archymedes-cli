...
beforeEach(async () => {
  root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-session-"));
});
...
describe("session storage", () => {
  ...
  it("lists sessions newest first and survives a corrupt file", async () => {
    ...
    await fs.writeFile(path.join(root, ".archymedes", "sessions", "broken.json"), "{ not json");
...
describe("checkpoints", () => {
  ...
  it("snapshots into a private index so the user's staged changes are untouched", async () => {
    ...
    const store = new CheckpointStore(root, path.join(root, ".archymedes", "index"), git);

    const checkpoint = await store.capture("add a flag", "turn_1", 0);
    expect(checkpoint?.tree).toBe("abc123");
    // Archymedes's own directory is excluded: staging it corrupts the very index git is writing.
    expect(calls[0]).toEqual(["add", "--all", "--", ".", ":(exclude).archymedes"]);
  ...
  it("stages before restoring, so files created after the snapshot are removed by the undo", async () => {
    ...
    expect(calls.at(-2)).toEqual(["add", "--all", "--", ".", ":(exclude).archymedes"]);
...
describe("checkpoints against a real repository", () => {
  it("detects a git repository, and reports its absence rather than failing", async () => {
    ...
    try {
      const outside = new CheckpointStore(repo, path.join(repo, ".archymedes", "index"));
      expect(await outside.isAvailable()).toBe(false);

      await runGit(["init", "-q"], { cwd: repo });
      expect(await new CheckpointStore(repo, path.join(repo, ".archymedes", "index")).isAvailable()).toBe(true);
    } finally {
  ...
  it("captures and restores real files through git plumbing", async () => {
    ...
    try {
      ...
      await runGit(["config", "user.email", "archymedes@test"], { cwd: repo });
      await runGit(["config", "user.name", "Archymedes"], { cwd: repo });
      ...
      const store = new CheckpointStore(repo, path.join(repo, ".archymedes", "checkpoint-index"));
