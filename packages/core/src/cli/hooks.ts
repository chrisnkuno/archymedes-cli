 ...
 *
 * Convention: `.archymedes/hooks/pre-tool-use/*` and `.archymedes/hooks/post-tool-use/*`, one executable script
 ...
 *
 * Discovery and execution both go through `ArchymedesWorkspace`, so a hook committed to a repository
...
export const HOOKS_DIRECTORY = ".archymedes/hooks";
const HOOK_TIMEOUT_MS = 5_000;

/** Every script directly under `phaseDirectory`, sorted for a deterministic run order. */
async function scriptsIn(workspace: ArchymedesWorkspace, phaseDirectory: string): Promise<string[]> {
...
function runHookScript(workspace: ArchymedesWorkspace, scriptPath: string, event: HookEvent): Promise<{ exitCode: number; stdout: string; stderr: string }> {
  ...
  // an injection path a fixed, quoting-free invocation avoids entirely. `ARCHYMEDES_HOOK_EVENT_B64` is
  ...
  // `ARCHYMEDES_HOOK_EVENT_B64=...` itself into the (nonexistent) program being run. `env` is a real
...
/**
 * One place hooks are discovered from — the top-level `.archymedes/hooks`, or a specific plugin's own.
...
async function scriptsFromSources(workspace: ArchymedesWorkspace, sources: readonly HookSource[], phase: "pre-tool-use" | "post-tool-use"): Promise<string[]> {
  const perSource = await Promise.all(sources.map((source) => scriptsIn(workspace, `${source}/${phase}`)));
  return perSource.flat();
}

/** Registry over a workspace's hook scripts, from one or more sources. Re-discovers on each phase call — hooks are cheap to list and a project's own scripts can change between turns without restarting Archymedes. */
export class HookRegistry {
  constructor(private readonly sources: readonly HookSource[], private readonly workspace: ArchymedesWorkspace) {}

  /** The top-level `.archymedes/hooks` directory only — the common case. */
  static local(workspace: ArchymedesWorkspace): HookRegistry {
   ...
   * from a bug in Archymedes unless something can show that it exists.
   */
