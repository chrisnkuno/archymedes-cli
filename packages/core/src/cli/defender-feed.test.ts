...
describe("central Defender feed replication", () => {
  ...
  beforeEach(async () => {
    directory = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-defender-feed-"));
    environment = { ARCHYMEDES_CONFIG_DIR: directory, ARCHYMEDES_DEFENDER_FEED_URL: "https://feed.example/manifest" };
  });
