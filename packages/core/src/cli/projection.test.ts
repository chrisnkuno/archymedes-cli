...
async function writeSession(sessionId: string, options: { decideApproval?: boolean } = {}): Promise<void> {
  ...
  await journal.append({
    ...
    request: {
      ...
      actionDigest: "digest-1", scopeKey: "archymedes-approval-v2:digest-1", policyVersion: "archymedes-approval-v2",
...
beforeEach(async () => {
  root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-projection-"));
...
describe("the projection is derived, never authoritative", () => {
  ...
  it("survives being deleted entirely, because it can always be rebuilt", async () => {
    ...
    await fs.rm(path.join(root, ".archymedes", "projection.db"), { force: true });
    projection = await SessionProjection.open(root);
  ...
  it("refuses to project a journal whose hash chain has been broken", async () => {
    ...
    await expect(projection.rebuild("s1")).rejects.toThrow(/integrity check failed|Invalid Archymedes event chain/);
  });
