...
export type ArchymedesToolOptions = {
  ...
  /** Skills, MCP servers and plugins — anything offering tools Archymedes did not ship with. See tool-providers.ts. */
   ...
   * `.archymedes/memory.md` inside a disposable sandbox is written to something that will be deleted.
   */
  memoryRoot?: string;
...
/**
 * The `scan_secrets` tool's own logic, exported so a UI surface (`archymedes.ts`'s `/scan`) can run the
 ...
 */
export async function scanWorkspaceForSecrets(workspace: Pick<ArchymedesWorkspace, "grep">, include?: string): Promise<PlacedSecretFinding[]> {
...
export async function createArchymedesTools(options: ArchymedesToolOptions): Promise<AgentTool[]> {
  ...
  const tools: AgentTool[] = [
    ...
    },
    {
      ...
      capabilityId: ARCHYMEDES_CAPABILITIES.terminal,
      effect: "workspace",
      requiresApproval: true,
      parallelSafe: false,
      async execute(args, context) {
        ...
        if (isLikelyPersistentCommand(command)) {
          return {
            content: "Refused a likely persistent foreground command. Use start_application so Archymedes can keep it alive, verify the HTTP endpoint, and return a reachable URL. Use a short timeout only for a temporary smoke test that should stop afterward.",
            isError: true,
        ...
        const verificationNote = rejectedVerification ? `\n\nArchymedes did not accept this as verification: ${rejectedVerification}` : "";
        return {
    ...
    },
    {
      ...
      async execute(args, context) {
        ...
        return {
          content: `Application ${status.id} is running and answered HTTP at ${status.url}. It will remain available while this Archymedes session is open.`,
          data: status,
    ...
    },
    {
      ...
      capabilityId: ARCHYMEDES_CAPABILITIES.playbooks,
      effect: "none",
      requiresApproval: false,
      parallelSafe: true,
      async execute(args) {
  ...
  if (options.search) {
    ...
    tools.push({
      ...
      capabilityId: ARCHYMEDES_CAPABILITIES.research,
      effect: "none",
      requiresApproval: false,
      parallelSafe: true,
      async execute(args) {
   ...
   * "persist durable conclusions to .archymedes/memory.md" with no tool to do so, which left it either
...
Written to ${scope === "project" ? ".archymedes/memory.md" : result.file} — the user can edit or delete it there.`
  ...
  if (options.externalToolProviders && options.externalToolProviders.length > 0) {
    ...
    for (const externalTool of await collectExternalTools(options.externalToolProviders)) {
      if (builtInNames.has(externalTool.name)) throw new Error(`Tool name '${externalTool.name}' collides with a built-in Archymedes tool`);
