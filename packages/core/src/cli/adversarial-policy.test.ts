...
import { actionDigest, ARCHYMEDES_CAPABILITIES } from "./permissions";
import { assessToolSafety } from "./safety";
import { isRefusedCommand } from "./tools";

/**
 * Deliberate attempts to defeat Archymedes's own defenses, run against the real functions rather than a
 * model of them.
 *
...
describe("approval digests resist tampering", () => {
  it("changes when a nested argument value changes, not just a top-level one", () => {
    const editTool = tool({ name: "edit_file", capabilityId: ARCHYMEDES_CAPABILITIES.write });
  ...
  it("changes when an array argument's contents or order changes", () => {
    const grepTool = tool({ name: "grep_files", capabilityId: ARCHYMEDES_CAPABILITIES.read });
  ...
  it("never lets the same tool and arguments collide across different capability scopes", () => {
    ...
    const asTerminal = actionDigest(call("run_command", args), tool({ name: "run_command", capabilityId: ARCHYMEDES_CAPABILITIES.terminal }));
    const asWrite = actionDigest(call("run_command", args), tool({ name: "run_command", capabilityId: ARCHYMEDES_CAPABILITIES.write }));
