/**
 * Trusted replication for Archymedes's centrally published Defensive Brain.
 *
...
import { archymedesConfigDirectory } from "./memory";
...
function feedRoot(environment: Record<string, string | undefined>): string {
  return path.join(archymedesConfigDirectory(environment), "defender-brain");
}
...
function parseManifest(value: unknown, now: number): DefenderFeedManifest {
  ...
  if (manifest.schemaVersion !== 1 || manifest.feed !== "archymedes-defender") throw new Error("unsupported manifest schema");
...
export async function refreshDefenderFeed(options: {
  ...
  const endpoint = options.environment.ARCHYMEDES_DEFENDER_FEED_URL?.trim();
  ...
  try {
    ...
    try {
      ...
      try { await fs.rename(temporary, corpusFile); }
        ...
        // Another Archymedes process may have won the same content-addressed activation race.
