...
beforeEach(async () => {
  root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-hooks-"));
});
...
/** Writes an executable script that decodes ARCHYMEDES_HOOK_EVENT_B64 and acts on it — a real script, not a mock. */
async function writeHookScript(scriptPath: string, body: string): Promise<void> {
  ...
  if (process.platform === "win32") {
    ...
    if (body.includes("payload=$(")) {
      windowsBody = `node -e "const p=Buffer.from(process.env.ARCHYMEDES_HOOK_EVENT_B64,'base64').toString();process.exit(p.includes('\\\"toolName\\\":\\\"run_command\\\"')&&p.includes('\\\"command\\\":\\\"rm -rf /tmp/x\\\"')?0:1)"`;
    }
...
describe("HookRegistry.local pre-tool-use", () => {
  ...
  it("lets an allowing hook (exit 0) through", async () => {
    await writeHookScript(path.join(root, ".archymedes/hooks/pre-tool-use/allow.sh"), "exit 0");
    const registry = HookRegistry.local(new LocalWorkspace(root));
    await expect(registry.runPreToolUse("write_file", { path: "a.txt" })).resolves.toEqual({ blocked: false });
  });

  it("blocks on a non-zero exit and surfaces the hook's own stderr as the reason", async () => {
    await writeHookScript(path.join(root, ".archymedes/hooks/pre-tool-use/deny.sh"), "echo 'no writes to secrets' >&2\nexit 1");
    const registry = HookRegistry.local(new LocalWorkspace(root));
  ...
  it("actually decodes the real event payload the hook receives on stdin's env var, not a fake one", async () => {
    // Proves the wiring end to end: a script that decodes ARCHYMEDES_HOOK_EVENT_B64 and checks the real
    // fields sees the real tool name and arguments this call was made with.
    await writeHookScript(
      path.join(root, ".archymedes/hooks/pre-tool-use/inspect.sh"),
      [
  ...
  it("runs multiple hooks in a deterministic (sorted) order and stops at the first block", async () => {
    ...
    await writeHookScript(path.join(root, ".archymedes/hooks/pre-tool-use/1-first.sh"), `echo first >> ${order}\nexit 0`);
    await writeHookScript(path.join(root, ".archymedes/hooks/pre-tool-use/2-second.sh"), `echo second >> ${order}\nexit 1`);
    await writeHookScript(path.join(root, ".archymedes/hooks/pre-tool-use/3-third.sh"), `echo third >> ${order}\nexit 0`);
    const registry = HookRegistry.local(new LocalWorkspace(root));
...
describe("HookRegistry.local post-tool-use", () => {
  ...
  it("collects a warning from a failing post-hook without throwing", async () => {
    await writeHookScript(path.join(root, ".archymedes/hooks/post-tool-use/audit.sh"), "echo 'file left uncommitted' >&2\nexit 1");
    const registry = HookRegistry.local(new LocalWorkspace(root));
