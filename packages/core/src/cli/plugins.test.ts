...
beforeEach(async () => {
  root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-plugins-"));
});
...
describe("discoverPlugins", () => {
  it("finds every plugin.json and pairs it with its own directory, and reports a missing directory as zero plugins", async () => {
    ...
    await fs.mkdir(path.join(root, ".archymedes/plugins/alpha"), { recursive: true });
    await fs.writeFile(path.join(root, ".archymedes/plugins/alpha/plugin.json"), JSON.stringify({ name: "alpha" }));
    await fs.mkdir(path.join(root, ".archymedes/plugins/beta"), { recursive: true });
    await fs.writeFile(path.join(root, ".archymedes/plugins/beta/plugin.json"), JSON.stringify({ name: "beta", mcpServers: [{ id: "s", command: "node" }] }));
    ...
    expect(alpha.directory).toBe(".archymedes/plugins/alpha");
  });
