...
import { DEFAULT_ARCHYMEDES_BUDGETS, ArchymedesAgent, type ArchymedesEvent, type ArchymedesTurnResult } from "./agent";
import { LocalWorkspace } from "./backends";
import { runGit, type GitRunner } from "./checkpoints";
import type { DaemonApprovalRequest, ArchymedesDaemonClient, ArchymedesSessionDaemon } from "./daemon";
import { capabilitiesForMode, type ArchymedesMode, type PermissionDecision } from "./permissions";
 ...
 *
 * A child is still a full session through `ArchymedesSessionDaemon` — same journal, same approval
 ...
 */
export function assertAttenuatedMode(parentMode: ArchymedesMode, childMode: ArchymedesMode): void {
...
export async function createChildWorktree(root: string, childId: string, git: GitRunner = runGit): Promise<ChildWorktree> {
  const branch = `archymedes-child/${childId}`;
  const worktreePath = path.join(root, ".archymedes", "children", childId);
...
export class ChildSessionRegistry {
  ...
  async spawn(options: SpawnChildOptions): Promise<ChildSession> {
    ...
    try {
      ...
      await client.open(({ onEvent, approve }) => new ArchymedesAgent({
        ...
        approve,
        onEvent,
        budgets: { ...DEFAULT_ARCHYMEDES_BUDGETS, maxRwf: options.maxRwf },
      }));
  ...
  async send(childId: string, objective: string): Promise<ArchymedesTurnResult> {
