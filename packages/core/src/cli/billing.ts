/**
 * Paying for Archymedes from the terminal.
 *
...
export function newIdempotencyKey(random: () => string = () => Math.random().toString(36).slice(2)): string {
  return `archymedes-topup-${Date.now().toString(36)}-${random()}${random()}`.slice(0, 64);
}
...
export function billingFromEnvironment(
  ...
  const baseUrl = environment.ARCHYMEDES_BILLING_URL?.trim();
  const apiKey = environment.ARCHYMEDES_BILLING_KEY?.trim();
...
export class CircuitPayGateway implements BillingGateway {
  ...
  constructor(private readonly options: CircuitPayOptions) {
    if (!options.baseUrl?.trim()) throw new BillingError("No billing URL configured. Set ARCHYMEDES_BILLING_URL.", "config");
    if (!options.apiKey?.trim()) throw new BillingError("No billing key configured. Set ARCHYMEDES_BILLING_KEY.", "config");
  ...
  async createCheckout(request: CheckoutRequest): Promise<Checkout> {
    ...
    const body = await this.call("POST", "/v1/checkouts", {
      ...
      body: { amount: request.amountRwf, currency: "RWF", purpose: "archymedes_credit_topup" },
...
function gatewayMessage(status: number, body: string): string {
  ...
  if (status === 401 || status === 403) return `The billing service rejected this key${detail ? ` (${detail})` : ""}. Check ARCHYMEDES_BILLING_KEY.`;
...
function readStatus(value: string): PaymentStatus {
  switch (value.toLowerCase()) {
    ...
    case "expired":
      return "expired";
    default:
      throw new BillingError(`The billing service reported a payment status Archymedes does not understand: ${value}`, "gateway");
  }
}
