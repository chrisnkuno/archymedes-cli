...
import type { ArchymedesAgent, ArchymedesEvent, ArchymedesTurnResult, RestoreScope } from "./agent";
...
export type DaemonNotification =
  ...
  | { protocolVersion: 1; type: "agent_event"; sessionId: string; event: ArchymedesEvent }
...
export type DaemonAgentFactory = (context: DaemonAgentFactoryContext) => ArchymedesAgent | Promise<ArchymedesAgent>;
...
type LiveSession = {
  agent: ArchymedesAgent;
  subscribers: Set<string>;
  tail: Promise<void>;
  commands: Map<string, Promise<ArchymedesTurnResult>>;
};
...
export class ArchymedesSessionDaemon {
  ...
  connect(options: {
    ...
    return new ArchymedesDaemonClient(this, id);
  }

  private publish(sessionId: string, notification: DaemonNotificationBody): void {
    const live = this.sessions.get(sessionId);
    if (!live) return;
    const complete = { protocolVersion: ARCHYMEDES_DAEMON_PROTOCOL_VERSION, ...notification } as DaemonNotification;
  ...
  send(clientId: string, sessionId: string, objective: string, commandId: string): Promise<ArchymedesTurnResult> {
    ...
    let resolve!: (result: ArchymedesTurnResult) => void;
    let reject!: (error: unknown) => void;
    const result = new Promise<ArchymedesTurnResult>((onResolve, onReject) => { resolve = onResolve; reject = onReject; });
...
export type DaemonSessionInfo = {
  ...
  workspaceKind: ArchymedesWorkspace["kind"];
  resumed: boolean;
};

/** Opaque client spoke. It owns no runtime or persisted session state. */
export class ArchymedesDaemonClient {
  private activeSession: string | null = null;
  private info: DaemonSessionInfo | null = null;

  constructor(private readonly daemon: ArchymedesSessionDaemon, readonly id: string) {}
  ...
  get workspaceKind(): ArchymedesWorkspace["kind"] { return this.info?.workspaceKind ?? "local"; }
  ...
  inspectTools(): ReturnType<ArchymedesAgent["inspectTools"]> { return this.daemon.inspectTools(this.id, this.sessionId); }
