 ...
 * instructed it to "persist durable conclusions to .archymedes/memory.md" with no tool to do so, so it
...
beforeEach(async () => {
  root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-remember-"));
  config = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-config-"));
  process.env.ARCHYMEDES_CONFIG_DIR = config;
});

afterEach(async () => {
  delete process.env.ARCHYMEDES_CONFIG_DIR;
  await fs.rm(root, { recursive: true, force: true });
  await fs.rm(config, { recursive: true, force: true });
});

async function rememberTool(): Promise<AgentTool> {
  const tools = await createArchymedesTools({ workspace: new LocalWorkspace(root), todos: new TodoList(), memoryRoot: root });
...
describe("the remember tool", () => {
  ...
  it("writes a project fact to the repository's own memory file", async () => {
    ...
    expect(result.data?.file).toBe(path.join(root, ".archymedes", "memory.md"));
  ...
  it("keeps user memory out of the repository", async () => {
    ...
    await expect(fs.readFile(path.join(root, ".archymedes", "memory.md"), "utf8")).rejects.toThrow();
  });
  ...
  it("tells the model where the fact went, so the user can be told where to edit it", async () => {
    ...
    expect(result.content).toContain(".archymedes/memory.md");
  ...
  it("writes a file the human-facing parser reads back unchanged", async () => {
    ...
    const raw = await fs.readFile(path.join(root, ".archymedes", "memory.md"), "utf8");
    expect(raw).toContain("# Archymedes memory");
