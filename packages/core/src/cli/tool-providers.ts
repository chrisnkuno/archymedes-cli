 ...
 * know about Archymedes's approval gating, capability scoping or provenance tagging; `toolsFromProvider`
 ...
 * what Archymedes's code needs from it.
 */
...
/**
 * Something that can list and run tools Archymedes did not ship with.
 *
 ...
 * is code Archymedes does not control, and a hang in it must not hang the whole agent turn. `timeoutMs` is
 * a parameter (not only the default) so a test can prove the timeout fires without waiting 30s for it.
 *
 * Two tools from different providers sharing a name is resolved by the caller (`createArchymedesTools`),
...
export async function toolsFromProvider(provider: ToolProvider, timeoutMs = DEFAULT_EXTERNAL_TOOL_TIMEOUT_MS): Promise<AgentTool[]> {
  ...
  return externalTools.map((externalTool) => {
    ...
    return {
      ...
      capabilityId: ARCHYMEDES_CAPABILITIES.external,
