...
/**
 * A skill: a small, named, project-local capability declared as data, not code Archymedes had to ship.
 *
 * Discovered from `.archymedes/skills/<name>/skill.json` — one directory per skill, so a skill can later
 * carry sibling files (a script `command` shells out to) without inventing a second convention.
 *
 * Both halves go through `ArchymedesWorkspace`: discovery via `listConfigFiles` (which deliberately
 * bypasses the ignored-directory list that hides `.archymedes` from the agent's own searches — see the
...
export const SKILLS_DIRECTORY = ".archymedes/skills";
 ...
 */
export async function discoverSkillManifestsIn(workspace: ArchymedesWorkspace, skillsDirectory: string): Promise<SkillManifest[]> {
...
/** Every `.archymedes/skills/<skill>/skill.json` in the workspace. */
export function discoverSkillManifests(workspace: ArchymedesWorkspace): Promise<SkillManifest[]> {
