...
beforeEach(async () => {
  root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-mcp-"));
...
describe("an MCP server's environment", () => {
  // Found by probing the running code, not by reading it: `run_command` already had Archymedes's own
  // provider keys stripped, while an MCP server — third-party code Archymedes spawns, with strictly less
