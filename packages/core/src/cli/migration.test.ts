...
import { EventJournal, ARCHYMEDES_PROTOCOL_VERSION, eventJournalPath, readEventJournal } from "./protocol";
 ...
 *
 * Every version constant in this package (`SESSION_SCHEMA_VERSION`, `ARCHYMEDES_PROTOCOL_VERSION`,
...
describe("session schema migration", () => {
  it("refuses a record from a schema version newer than this code understands", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-migration-"));
    try {
...
describe("event journal protocol migration", () => {
  it("fails loud on an unsupported protocol version rather than misreading the events", async () => {
    ...
    try {
      ...
      event.protocolVersion = ARCHYMEDES_PROTOCOL_VERSION + 1;
