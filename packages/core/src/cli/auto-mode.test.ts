...
import { PermissionLedger, ARCHYMEDES_CAPABILITIES, capabilitiesForMode } from "./permissions";
...
describe("auto mode", () => {
  ...
  it("gives auto mode the tools to actually work with", () => {
    ...
    const working = Object.values(ARCHYMEDES_CAPABILITIES).filter((capability) => capability !== ARCHYMEDES_CAPABILITIES.playbooks);
