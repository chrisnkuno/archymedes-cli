 ...
 * the same tree the agent is already working in, which is why this writes through `ArchymedesWorkspace`
...
/** Root-relative, forward-slashed: the same shape every workspace path in Archymedes has. */
export const ARTIFACT_DIRECTORY = ".archymedes/artifacts";
...
export class WorkspaceArtifactStore implements ToolResultArtifactStore {
  ...
  constructor(private readonly workspace: Pick<ArchymedesWorkspace, "writeFile">, private readonly maxBytes = MAX_ARTIFACT_BYTES) {}
