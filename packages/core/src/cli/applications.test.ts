...
describe("LocalApplicationSupervisor", () => {
  ...
  async function fixture(source: string): Promise<{ root: string; supervisor: LocalApplicationSupervisor }> {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-application-"));
    roots.push(root);
  ...
  it("keeps an HTTP application reachable until it is explicitly stopped", async () => {
    ...
    const { supervisor } = await fixture(`
      ...
      const server = http.createServer((_request, response) => response.end("archymedes-preview-ok"));
    ...
    expect(await (await fetch(started.url)).text()).toBe("archymedes-preview-ok");
