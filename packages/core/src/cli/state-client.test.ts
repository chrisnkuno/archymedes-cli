...
import { ArchymedesStateClient, resolveArchymedesStateBinary, type StateProcess } from "./state-client";

const clients: ArchymedesStateClient[] = [];
...
describe("ArchymedesStateClient", () => {
  it("multiplexes versioned JSONL requests over one shell-free child", async () => {
    const client = await ArchymedesStateClient.connect({ binary: "fake-state", processFactory: successProcess });
    clients.push(client);
  ...
  it("surfaces protocol errors as typed failures", async () => {
    ...
    await expect(ArchymedesStateClient.connect({ binary: "fake-state", processFactory })).rejects.toMatchObject({ code: "broken", message: "bad source" });
  });

  it("times out a wedged sidecar instead of hanging Archymedes", async () => {
    await expect(ArchymedesStateClient.connect({ binary: "fake-state", processFactory: () => fakeProcess(() => null), requestTimeoutMs: 30 })).rejects.toMatchObject({ code: "timeout" });
  });
  ...
  it("terminates a sidecar that emits a malformed protocol response", async () => {
    ...
    await expect(ArchymedesStateClient.connect({ binary: "fake-state", processFactory })).rejects.toThrow(/malformed protocol response/);
...
describe("native binary resolution", () => {
  it("honours an explicit executable without modifying it", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-state-resolve-"));
    ...
    await expect(resolveArchymedesStateBinary({ ...process.env, ARCHYMEDES_STATE_BINARY: binary })).resolves.toBe(path.resolve(binary));
    await fs.rm(directory, { recursive: true, force: true });
  });

  it("fails clearly when an explicit override is unusable", async () => {
    await expect(resolveArchymedesStateBinary({ ...process.env, ARCHYMEDES_STATE_BINARY: path.join(os.tmpdir(), "missing-archymedes-state") })).rejects.toThrow(/not executable/);
  });
});
