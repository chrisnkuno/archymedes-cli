 ...
 * it in one of them is worth exactly as much in the others. It used to sit in `archymedes-cli`, which is
 ...
 * - It is scoped. Project memory (`.archymedes/memory.md`) belongs to the repository and can be committed
...
export const MEMORY_HEADER = "# Archymedes memory";

const PREAMBLE = [
  MEMORY_HEADER,
  "",
  "Durable facts Archymedes is told to remember. One per line. Edit or delete freely — this file is the",
  "record, not a cache of one.",
  "",
].join("\n");

/**
 * Where Archymedes keeps its configuration, and therefore user-scope memory.
 *
 ...
 */
export function archymedesConfigDirectory(environment: Record<string, string | undefined>, platform: NodeJS.Platform = process.platform): string {
...
export function memoryFile(scope: MemoryScope, root: string, environment: Record<string, string | undefined>): string {
  return scope === "project"
    ...
    : path.join(archymedesConfigDirectory(environment), "memory.md");
}
 ...
 * every turn would bill the user for the same paragraph on every message. `archymedes.ts` re-sends it
