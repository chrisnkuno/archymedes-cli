...
beforeEach(async () => {
  root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-skills-"));
});
...
describe("discoverSkillManifests", () => {
  it("finds every skill.json under .archymedes/skills and reports a missing directory as zero skills", async () => {
    ...
    await fs.writeFile(path.join(root, ".archymedes/skills/greet/skill.json"), JSON.stringify({ name: "greet", description: "d", command: "echo hi", inputSchema: validSchema }));
    await fs.mkdir(path.join(root, ".archymedes/skills/count"), { recursive: true });
    await fs.writeFile(path.join(root, ".archymedes/skills/count/skill.json"), JSON.stringify({ name: "count", description: "d2", command: "echo 1", inputSchema: validSchema }));
  ...
  it("reports a parse failure for one bad manifest rather than silently skipping it", async () => {
    await fs.mkdir(path.join(root, ".archymedes/skills/broken"), { recursive: true });
    await fs.writeFile(path.join(root, ".archymedes/skills/broken/skill.json"), "not json");
