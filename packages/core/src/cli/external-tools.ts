 ...
 * `ArchymedesAgent` has one call to make rather than reimplementing plugin unbundling itself.
 *
 * Everything is discovered and executed through `ArchymedesWorkspace`, so a `.archymedes` directory committed to
 * a repository behaves identically whether the session runs locally, in E2B, or in Docker. The one
 * thing that is not workspace-relative is an MCP server *process*: `.archymedes/mcp.json` says which
 * servers to start, but Archymedes spawns them on the machine Archymedes itself runs on, which is the same
 * machine either way.
 *
...
/**
 * The one provider that is always present rather than configured: the top-level `.archymedes/skills`
