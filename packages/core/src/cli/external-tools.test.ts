...
beforeEach(async () => {
  root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-external-"));
});
...
describe("loadLocalExternalTooling", () => {
  it("returns a working, empty assembly for a project with no .archymedes directory at all", async () => {
  ...
  it("discovers top-level skills and a plugin's bundled skills as separately-identified providers", async () => {
    await writeSkill(path.join(root, ".archymedes/skills"), "top", "printf top");
    await fs.mkdir(path.join(root, ".archymedes/plugins/demo"), { recursive: true });
    await fs.writeFile(path.join(root, ".archymedes/plugins/demo/plugin.json"), JSON.stringify({ name: "demo" }));
    await writeSkill(path.join(root, ".archymedes/plugins/demo/skills"), "bundled", "printf bundled");
  ...
  it("runs both a top-level hook and a plugin's own hook for the same call", async () => {
    ...
    await writeHook(path.join(root, ".archymedes/hooks/pre-tool-use"), "top.sh", `echo top >> ${log}\nexit 0`);
    await fs.mkdir(path.join(root, ".archymedes/plugins/demo"), { recursive: true });
    await fs.writeFile(path.join(root, ".archymedes/plugins/demo/plugin.json"), JSON.stringify({ name: "demo" }));
    await writeHook(path.join(root, ".archymedes/plugins/demo/hooks/pre-tool-use"), "bundled.sh", `echo bundled >> ${log}\nexit 0`);
  ...
  it("lets a plugin's own hook block a call, exactly as a top-level hook can", async () => {
    await fs.mkdir(path.join(root, ".archymedes/plugins/demo"), { recursive: true });
    await fs.writeFile(path.join(root, ".archymedes/plugins/demo/plugin.json"), JSON.stringify({ name: "demo" }));
    await writeHook(path.join(root, ".archymedes/plugins/demo/hooks/pre-tool-use"), "deny.sh", "echo 'plugin says no' >&2\nexit 1");
  ...
  it("surfaces a malformed plugin manifest rather than loading the rest and staying silent", async () => {
    await fs.mkdir(path.join(root, ".archymedes/plugins/broken"), { recursive: true });
    await fs.writeFile(path.join(root, ".archymedes/plugins/broken/plugin.json"), "not json");
