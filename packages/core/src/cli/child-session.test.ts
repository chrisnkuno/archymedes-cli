...
import { ArchymedesSessionDaemon } from "./daemon";
import {
  ChildBudgetError,
  ChildBudgetPool,
  ChildMailbox,
...
async function initRepo(): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "archymedes-child-repo-"));
...
describe("ChildSessionRegistry, a full parent/child lifecycle", () => {
  ...
  it("two concurrent children editing the same filename never collide", async () => {
    ...
    // ArchymedesAgent instances through a real ArchymedesSessionDaemon, actually running.
  ...
  it("refuses to spawn a child the parent's pool cannot afford, and leaves no worktree behind", async () => {
    ...
    const leaked = await fs.readdir(path.join(root, ".archymedes", "children")).catch(() => []);
