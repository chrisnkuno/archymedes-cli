/**
 * Native, bounded retrieval over Archymedes's reviewed defensive-security corpus.
 * The reviewed JSONL is authoritative; SQLite is only a disposable, token-saving read model.
 */
import { access } from "node:fs/promises";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { tryConnectArchymedesState, type DefenderBrainHit, type ArchymedesStateClient } from "./state-client";
import { activeDefenderCorpusDirectory } from "./defender-feed";

type BrainClient = Pick<ArchymedesStateClient, "rebuildDefenderBrain" | "searchDefenderBrain" | "close">;
...
export function defenderKnowledgeCandidates(moduleUrl = import.meta.url): string[] {
  ...
  return [
    ...
    path.resolve(directory, "../../../archymedes-state/defender-knowledge"),
  ];
}
...
export class DefenderBrain {
  ...
  constructor(
    ...
    private readonly connect: () => Promise<BrainClient | null> = () => tryConnectArchymedesState(),
  ...
  private ensureReady(): Promise<boolean> {
    this.initialized ??= (async () => {
      try {
        ...
        if (!this.client) throw new Error("native Archymedes State package is unavailable on this platform");
