...
/**
 * Where a tool came from: Archymedes's own fixed set, or something loaded at runtime.
 *
 * Optional and absent on every built-in tool literal — adding a required field to every entry in
 * `createArchymedesTools` for a distinction only externally-sourced tools need would be exactly the kind
...
export type AgentRuntimeEvent =
  ...
  // like Archymedes has frozen, and Ctrl+C becomes the only feedback mechanism the user can discover.
