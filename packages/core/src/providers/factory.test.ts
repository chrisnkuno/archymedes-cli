...
import { createAgentTurnProvider, createCodingModelProvider, createCodingSandboxProvider, createDockerProvider, createE2BProvider, createModelPriceCatalog, createOpenAIProvider } from "./factory";
...
describe("provider factory", () => {
  ...
  it("selects Archymedes messaging's turn provider explicitly", () => {
  ...
  it("keeps Docker disabled until a coding image is configured", () => {
    ...
    expect(createDockerProvider({ DOCKER_CODING_IMAGE: "circuit-archymedes-coding:latest" })).toBeInstanceOf(DockerSandboxProvider);
  });
  ...
  it("switches to the Docker backend only on explicit selection", () => {
    expect(createCodingSandboxProvider({ CODING_SANDBOX_PROVIDER: "docker", DOCKER_CODING_IMAGE: "circuit-archymedes-coding:latest" })).toBeInstanceOf(DockerSandboxProvider);
  ...
  it("parses a model price catalog from the environment", () => {
    expect(createModelPriceCatalog({ MODEL_INPUT_PER_MILLION: "2000", MODEL_OUTPUT_PER_MILLION: "8000" })).toEqual({
      inputRatePerMillion: 2_000,
      outputRatePerMillion: 8_000,
    });
    expect(() => createModelPriceCatalog({ MODEL_INPUT_PER_MILLION: "-1", MODEL_OUTPUT_PER_MILLION: "8000" })).toThrow("positive");
  });
});
