...
export class DockerSandboxProvider implements InteractiveCodingSandboxProvider {
  ...
  async createSandbox(request: SandboxRequest): Promise<SandboxSession> {
    ...
    const containerName = `circuit-archymedes-${request.taskId.replace(/[^a-zA-Z0-9_.-]/g, "_").slice(0, 40)}-${randomUUID().slice(0, 8)}`;
    const result = await this.run([
      ...
      "--label", `circuit-archymedes-task=${request.taskId}`,
      this.options.image,
  ...
  async writeFile(sandboxId: string, path: string, content: string): Promise<void> {
    ...
    const tempDir = await mkdtemp(join(tmpdir(), "circuit-archymedes-docker-"));
    try {
