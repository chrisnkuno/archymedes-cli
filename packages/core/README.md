...
The provider-neutral coding-agent core behind [`archymedes-cli`](https://www.npmjs.com/package/archymedes-cli): a bounded tool-use runtime, local and sandboxed workspace backends, model adapters for Anthropic/OpenAI-compatible providers, and multi-currency cost accounting. Use this package directly if you're embedding an agent loop in your own tool rather than running Archymedes's CLI.

```bash
npm install @archymedes/core
```
...
- **Durable state** — integrity-checked JSON snapshots and hash-chained journals remain canonical; the optional `archymedes-state` sidecar provides rebuildable SQLite/FTS5 recall.
...
```ts
import { resolveProvider } from "@archymedes/core";
...
// Supply resolved.provider to BoundedAgentRuntime or ArchymedesAgent. OPENAI_BASE_URL may point at an
...
- **`resolveProvider` now honours `ARCHYMEDES_PROVIDER`.** With no explicit `provider` option, a valid, configured `ARCHYMEDES_PROVIDER` in the environment wins over the previous "first configured provider in catalog order" rule. If you embed this package in a process where that variable is set for unrelated reasons, resolution changes; pass `provider` explicitly to pin it. An unset, unrecognised, or unconfigured value falls back to the old behaviour rather than erroring, so a stale setting can never stop a session starting.
...
- **`createArchymedesTools` is now `async`.** It awaits externally-sourced tools (skills, MCP servers, plugins) before returning. `const tools = createArchymedesTools(…)` becomes `const tools = await createArchymedesTools(…)`.
- **`ArchymedesWorkspace` gained two required members.** A custom implementation needs `listConfigFiles(prefix)` — files under a prefix, ignoring the ignored-directory list, `[]` when the directory is absent — and `commandPlatform`, the platform whose shell rules apply to commands it runs (`process.platform` for a local workspace, `"linux"` for anything containerised).
- **Discovery takes a workspace, not a root path.** `discoverSkillManifests`, `discoverPlugins`, `discoverMcpServers`, `loadLocalExternalTooling` and `HookRegistry.local` now receive a `ArchymedesWorkspace`, which is what lets a `.archymedes` directory work in a sandbox rather than only on the host.
- **Stored approvals are void.** `APPROVAL_POLICY_VERSION` moved to `archymedes-approval-v2` because a tool's provenance now forms part of its action digest — a standing approval for a built-in `run_command` must not silently cover a same-named tool an MCP server starts offering. Persisted `allow_always` decisions from v1 are ignored and will be asked again once.
