...
| WS-2.9 | Extract the transcript render state (`style`, palette/depth/glyph state, `configureRendering`, `renderEvent` and helpers) → `app/` module with explicit reset calls | todo | `archymedes.ts` | `bun run recheck --pty` | |
