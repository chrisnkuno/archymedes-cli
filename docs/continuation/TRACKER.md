...
| WS-2.11 | Extract slash-command handlers from `main()` one per task, behind explicit context types (largest first: `/mode` block, `/diff`, `/cat`, `/workspace`, `/update`, `/watch`, `/cost`, `/scan`; 26 inline handlers in total) | todo | `archymedes.ts` → `commands/` or `app/` | `bun run recheck --pty` per handler | |
